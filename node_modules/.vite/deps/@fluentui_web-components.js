//#region node_modules/@microsoft/fast-element/dist/esm/platform.js
/**
...
if ($global.trustedTypes === void 0) $global.trustedTypes = { createPolicy: (n, r) => r };
var propConfig = {
	configurable: false,
	enumerable: false,
	writable: false
};
if ($global.FAST === void 0) Reflect.defineProperty($global, "FAST", Object.assign({ value: Object.create(null) }, propConfig));
/**
* The FAST global.
* @internal
*/
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/dom.js
var updateQueue = $global.FAST.getById(1, () => {
	...
	function enqueue(callable) {
		if (tasks.length < 1) $global.requestAnimationFrame(process);
		tasks.push(callable);
	}
...
var fastHTMLPolicy = $global.trustedTypes.createPolicy("fast-html", { createHTML: (html) => html });
...
var DOM = Object.freeze({
	...
	*/
	setHTMLPolicy(policy) {
		if (htmlPolicy !== fastHTMLPolicy) throw new Error("The HTML policy can only be set once.");
		htmlPolicy = policy;
	},
	/**
	...
	*/
	setAttribute(element, attributeName, value) {
		if (value === null || value === void 0) element.removeAttribute(attributeName);
	...
	*/
	removeChildNodes(parent) {
		for (let child = parent.firstChild; child !== null; child = parent.firstChild) parent.removeChild(child);
	},
	/**
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/observation/notifier.js
/**
	...
	*/
	notify(args) {
		...
		if (spillover === void 0) {
			...
			if (sub1 !== void 0) sub1.handleChange(source, args);
			if (sub2 !== void 0) sub2.handleChange(source, args);
		} else for (let i = 0, ii = spillover.length; i < ii; ++i) spillover[i].handleChange(source, args);
	}
};
/**
...
var PropertyChangeNotifier = class {
	...
	*/
	notify(propertyName) {
		...
		if (subscribers !== void 0) subscribers.notify(propertyName);
		(_a = this.sourceSubscribers) === null || _a === void 0 || _a.notify(propertyName);
	}
	/**
	* Subscribes to notification of changes in an object's state.
	* @param subscriber - The object that is subscribing for change notification.
	...
	*/
	subscribe(subscriber, propertyToWatch) {
		...
		if (propertyToWatch) {
			...
			if (subscribers === void 0) this.subscribers[propertyToWatch] = subscribers = new SubscriberSet(this.source);
	...
	*/
	unsubscribe(subscriber, propertyToUnwatch) {
		...
		if (propertyToUnwatch) {
			...
			if (subscribers !== void 0) subscribers.unsubscribe(subscriber);
		} else (_a = this.sourceSubscribers) === null || _a === void 0 || _a.unsubscribe(subscriber);
	}
};
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/observation/observable.js
/**
...
var Observable = FAST.getById(2, () => {
	...
	function getNotifier(source) {
		...
		if (found === void 0) if (Array.isArray(source)) found = createArrayObserver(source);
		else notifierLookup.set(source, found = new PropertyChangeNotifier(source));
		return found;
	}
	...
	class DefaultObservableAccessor {
		...
		setValue(source, newValue) {
			...
			if (oldValue !== newValue) {
				...
				if (typeof callback === "function") callback.call(source, oldValue, newValue);
	...
	class BindingObserverImplementation extends SubscriberSet {
		...
		observe(source, context) {
			if (this.needsRefresh && this.last !== null) this.disconnect();
	...
	return Object.freeze({
		...
		*/
		track(source, propertyName) {
			if (watcher !== void 0) watcher.watch(source, propertyName);
		},
		/**
		...
		*/
		trackVolatile() {
			if (watcher !== void 0) watcher.needsRefresh = true;
		},
		/**
		...
		*/
		defineProperty(target, nameOrAccessor) {
			if (typeof nameOrAccessor === "string") nameOrAccessor = new DefaultObservableAccessor(nameOrAccessor);
...
function volatile(target, name, descriptor) {
	return Object.assign({}, descriptor, { get: function() {
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/templating/html-directive.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/templating/binding.js
function normalBind(source, context) {
	this.source = source;
	this.context = context;
	if (this.bindingObserver === null) this.bindingObserver = Observable.binding(this.binding, this, this.isBindingVolatile);
...
function updateContentTarget(value) {
	...
	if (value.create) {
		...
		else if (this.target.$fastTemplate !== value) {
			if (view.isComposed) {
				view.remove();
				view.unbind();
			}
...
function updateClassTarget(value) {
	...
	if (version === 0) return;
	version -= 1;
	for (const name in classVersions) if (classVersions[name] === version) target.classList.remove(name);
}
/**
	...
	set targetName(value) {
		...
		switch (value[0]) {
			...
			default:
				...
				if (value === "class") this.updateTarget = updateClassTarget;
				break;
		}
	}
	/**
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/templating/compiler.js
var sharedContext = null;
var CompilationContext = class CompilationContext {
	addFactory(factory) {
...
function parseContent(context, value) {
	...
	if (valueParts.length === 1) return null;
	const bindingParts = [];
	for (let i = 0, ii = valueParts.length; i < ii; ++i) {
		...
		if (literal !== "") bindingParts.push(literal);
	}
	return bindingParts;
}
function compileAttributes(context, node, includeBasicValues = false) {
	...
	for (let i = 0, ii = attributes.length; i < ii; ++i) {
		...
		} else result = createAggregateBinding(parseResult);
		if (result !== null) {
...
function compileContent(context, node, walker) {
	...
	if (parseResult !== null) {
		...
		for (let i = 0, ii = parseResult.length; i < ii; ++i) {
			...
			if (typeof currentPart === "string") currentNode.textContent = currentPart;
			else {
			...
			context.targetIndex++;
			if (currentNode !== node) walker.nextNode();
		}
		context.targetIndex--;
	}
}
	...
	while (node = walker.nextNode()) {
		...
		switch (node.nodeType) {
			...
			case 3:
				...
				break;
			case 8: if (DOM.isMarker(node)) context.addFactory(directives[DOM.extractDirectiveIndexFromMarker(node)]);
		}
	}
	let targetOffset = 0;
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/templating/view.js
...
var HTMLView = class {
	...
	*/
	dispose() {
		...
		for (let i = 0, ii = behaviors.length; i < ii; ++i) behaviors[i].unbind(oldSource);
	}
	/**
	...
	*/
	bind(source, context) {
		...
		if (this.source === source) return;
			...
			this.source = source;
			this.context = context;
			for (let i = 0, ii = behaviors.length; i < ii; ++i) behaviors[i].bind(source, context);
		}
	}
	/**
	...
	*/
	static disposeContiguousBatch(views) {
		...
		for (let i = 0, ii = views.length; i < ii; ++i) {
			...
			for (let j = 0, jj = behaviors.length; j < jj; ++j) behaviors[j].unbind(oldSource);
		}
	}
};
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/templating/template.js
/**
...
var ViewTemplate = class {
	...
	create(hostBindingTarget) {
		if (this.fragment === null) {
			...
			if (typeof html === "string") {
				...
				if (fec !== null && fec.tagName === "TEMPLATE") template = fec;
			} else template = html;
		...
		for (let ii = viewFactories.length; behaviorIndex < ii; ++behaviorIndex) {
			...
			while (node !== null) if (targetIndex === factoryIndex) {
		...
		if (this.hasHostBehaviors) {
			...
			for (let i = 0, ii = hostFactories.length; i < ii; ++i, ++behaviorIndex) behaviors[behaviorIndex] = hostFactories[i].createBehavior(hostBindingTarget);
		}
	...
	*/
	render(source, host, hostBindingTarget) {
		...
		if (hostBindingTarget === void 0) hostBindingTarget = host;
...
function html(strings, ...values) {
	...
	for (let i = 0, ii = strings.length - 1; i < ii; ++i) {
		...
		if (typeof value === "function") value = new HTMLBindingDirective(value);
		if (value instanceof TargetedHTMLDirective) {
			const match = lastAttributeNameRegex.exec(currentString);
			if (match !== null) value.targetName = match[2];
		}
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/styles/element-styles.js
/**
...
if (DOM.supportsAdoptedStyleSheets) try {
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/components/attributes.js
/**
...
var booleanConverter = {
	...
	fromView(value) {
		if (value === null || value === void 0 || value === "false" || value === false || value === 0) return false;
		return true;
	}
};
/**
...
* @public
*/
var AttributeDefinition = class AttributeDefinition {
	/**
	...
	*/
	constructor(Owner, name, attribute = name.toLowerCase(), mode = "reflect", converter) {
		...
		if (mode === "boolean" && converter === void 0) this.converter = booleanConverter;
	}
	/**
	...
	static collect(Owner, ...attributeLists) {
		...
		for (let i = 0, ii = attributeLists.length; i < ii; ++i) {
			...
			for (let j = 0, jj = list.length; j < jj; ++j) {
				const config = list[j];
				if (typeof config === "string") attributes.push(new AttributeDefinition(Owner, config));
...
function attr(configOrTarget, prop) {
	...
	function decorator($target, $prop) {
		if (arguments.length > 1) config.property = $prop;
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/components/fast-definitions.js
...
var FASTElementDefinition = class {
	...
	*/
	constructor(type, nameOrConfig = type.definition) {
		if (typeof nameOrConfig === "string") nameOrConfig = { name: nameOrConfig };
		this.type = type;
	...
	*/
	define(registry = customElements) {
		...
		if (fastRegistry.register(this)) {
			...
			for (let i = 0, ii = attributes.length; i < ii; ++i) Observable.defineProperty(proto, attributes[i]);
			Reflect.defineProperty(type, "observedAttributes", {
				value: this.observedAttributes,
				enumerable: true
			});
		}
		if (!registry.get(this.name)) registry.define(this.name, type, this.elementOptions);
		return this;
	}
};
/**
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/components/controller.js
...
* @public
*/
var Controller = class Controller extends PropertyChangeNotifier {
	/**
	...
	*/
	...
	set styles(value) {
		if (this._styles === value) return;
		if (this._styles !== null) this.removeStyles(this._styles);
		this._styles = value;
		if (!this.needsInitialization && value !== null) this.addStyles(value);
	}
	/**
	...
	*/
	addStyles(styles) {
		...
		if (styles instanceof HTMLStyleElement) target.append(styles);
	...
	*/
	removeStyles(styles) {
		const target = getShadowRoot(this.element) || this.element.getRootNode();
		if (styles instanceof HTMLStyleElement) target.removeChild(styles);
		else if (styles.isAttachedTo(target)) {
			const sourceBehaviors = styles.behaviors;
			styles.removeStylesFrom(target);
			if (sourceBehaviors !== null) this.removeBehaviors(sourceBehaviors);
		}
	}
	/**
	...
	*/
	addBehaviors(behaviors) {
		...
		for (let i = 0; i < length; ++i) {
			...
			if (targetBehaviors.has(behavior)) targetBehaviors.set(behavior, targetBehaviors.get(behavior) + 1);
			else {
		...
		if (this._isConnected) {
			...
			for (let i = 0; i < behaviorsToBind.length; ++i) behaviorsToBind[i].bind(element, defaultExecutionContext);
		}
	}
	/**
	...
	*/
	removeBehaviors(behaviors, force = false) {
		...
		if (this._isConnected) {
			const element = this.element;
			for (let i = 0; i < behaviorsToUnbind.length; ++i) behaviorsToUnbind[i].unbind(element);
		}
	}
	/**
	...
	*/
	onConnectedCallback() {
		...
		const element = this.element;
		if (this.needsInitialization) this.finishInitialization();
		else if (this.view !== null) this.view.bind(element, defaultExecutionContext);
		const behaviors = this.behaviors;
		if (behaviors !== null) for (const [behavior] of behaviors) behavior.bind(element, defaultExecutionContext);
	...
	*/
	onDisconnectedCallback() {
		...
		if (behaviors !== null) {
			const element = this.element;
			for (const [behavior] of behaviors) behavior.unbind(element);
		}
	}
	/**
	...
	*/
	onAttributeChangedCallback(name, oldValue, newValue) {
		...
		if (attrDef !== void 0) attrDef.onAttributeChangedCallback(this.element, newValue);
	}
	/**
	...
	*/
	emit(type, detail, options) {
		if (this._isConnected) return this.element.dispatchEvent(new CustomEvent(type, Object.assign(Object.assign({ detail }, defaultEventOptions), options)));
		return false;
	}
	finishInitialization() {
		...
		if (this._template === null) {
			if (this.element.resolveTemplate) this._template = this.element.resolveTemplate();
			else if (definition.template) this._template = definition.template || null;
		}
		if (this._template !== null) this.renderTemplate(this._template);
		if (this._styles === null) {
			if (this.element.resolveStyles) this._styles = this.element.resolveStyles();
			else if (definition.styles) this._styles = definition.styles || null;
		}
		if (this._styles !== null) this.addStyles(this._styles);
	...
	renderTemplate(template) {
		...
		if (this.view !== null) {
			this.view.dispose();
			this.view = null;
		} else if (!this.needsInitialization) DOM.removeChildNodes(host);
		if (template) this.view = template.render(element, host, element);
	}
	/**
	...
	*/
	static forCustomElement(element) {
		...
		if (definition === void 0) throw new Error("Missing FASTElement definition.");
		return element.$fastController = new Controller(element, definition);
	}
};
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/components/fast-element.js
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/styles/css-directive.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/styles/css.js
...
function css(strings, ...values) {
	...
	if (behaviors.length) elementStyles.withBehaviors(...behaviors);
...
var CSSPartial = class extends CSSDirective {
	constructor(styles, behaviors) {
		...
		const stylesheets = styles.reduce((accumulated, current) => {
			if (typeof current === "string") this.css += current;
			else accumulated.push(current);
			return accumulated;
		}, []);
		if (stylesheets.length) this.styles = ElementStyles.create(stylesheets);
	}
	createBehavior() {
		return this;
	}
	...
	bind(el) {
		if (this.styles) el.$fastController.addStyles(this.styles);
		if (this.behaviors.length) el.$fastController.addBehaviors(this.behaviors);
	}
	unbind(el) {
		if (this.styles) el.$fastController.removeStyles(this.styles);
		if (this.behaviors.length) el.$fastController.removeBehaviors(this.behaviors);
	}
};
/**
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/observation/array-change-records.js
/** @internal */
...
function calcEditDistances(current, currentStart, currentEnd, old, oldStart, oldEnd) {
	...
	for (let i = 1; i < rowCount; ++i) for (let j = 1; j < columnCount; ++j) if (current[currentStart + j - 1] === old[oldStart + i - 1]) distances[i][j] = distances[i - 1][j - 1];
	else {
...
function sharedPrefix(current, old, searchLength) {
	for (let i = 0; i < searchLength; ++i) if (current[i] !== old[i]) return i;
	return searchLength;
}
...
function calcSplices(current, currentStart, currentEnd, old, oldStart, oldEnd) {
	...
	oldEnd -= suffixCount;
	if (currentEnd - currentStart === 0 && oldEnd - oldStart === 0) return emptyArray;
	...
	for (let i = 0; i < ops.length; ++i) switch (ops[i]) {
		case EDIT_LEAVE:
			...
			break;
		case EDIT_UPDATE:
			if (splice === void 0) splice = newSplice(index, [], 0);
			splice.addedCount++;
			index++;
...
function mergeSplice(splices, index, removed, addedCount) {
	...
	for (let i = 0; i < splices.length; i++) {
		...
		if (intersectCount >= 0) {
			...
			if (!splice.addedCount && !deleteCount) inserted = true;
			else {
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/observation/array-observer.js
var arrayObservationEnabled = false;
function adjustIndex(changeRecord, array) {
	let index = changeRecord.index;
	const arrayLength = array.length;
	if (index > arrayLength) index = arrayLength - changeRecord.addedCount;
...
var ArrayObserver = class extends SubscriberSet {
	...
	addSplice(splice) {
		if (this.splices === void 0) this.splices = [splice];
...
function enableArrayObservation() {
	if (arrayObservationEnabled) return;
	arrayObservationEnabled = true;
	Observable.setArrayObserverFactory((collection) => {
		return new ArrayObserver(collection);
	});
	const proto = Array.prototype;
	if (proto.$fastPatch) return;
	...
	proto.pop = function() {
		...
		if (o !== void 0 && notEmpty) o.addSplice(newSplice(this.length, [methodCallResult], 0));
		return methodCallResult;
	};
	proto.push = function() {
		const methodCallResult = push.apply(this, arguments);
		const o = this.$fastController;
		if (o !== void 0) o.addSplice(adjustIndex(newSplice(this.length - arguments.length, [], arguments.length), this));
		return methodCallResult;
	};
	...
	proto.shift = function() {
		...
		const o = this.$fastController;
		if (o !== void 0 && notEmpty) o.addSplice(newSplice(0, [methodCallResult], 0));
		return methodCallResult;
	};
	proto.sort = function() {
		let oldArray;
	...
	proto.splice = function() {
		...
		const o = this.$fastController;
		if (o !== void 0) o.addSplice(adjustIndex(newSplice(+arguments[0], methodCallResult, arguments.length > 2 ? arguments.length - 2 : 0), this));
		return methodCallResult;
	};
	proto.unshift = function() {
		const methodCallResult = unshift.apply(this, arguments);
		const o = this.$fastController;
		if (o !== void 0) o.addSplice(adjustIndex(newSplice(0, [], arguments.length), this));
		return methodCallResult;
	};
}
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/templating/ref.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/interfaces.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/templating/when.js
var noTemplate = () => null;
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/templating/repeat.js
...
var RepeatBehavior = class {
	...
	*/
	constructor(location, itemsBinding, isItemsBindingVolatile, templateBinding, isTemplateBindingVolatile, options) {
		...
		if (options.positioning) this.bindView = bindWithPositioning;
	}
	/**
	* Bind this behavior to the source.
	* @param source - The source to bind to.
	...
	*/
	...
	updateViews(splices) {
		...
		if (this.options.positioning) for (let i = 0, ii = views.length; i < ii; ++i) {
...
function repeat(itemsBinding, templateOrTemplateBinding, options = defaultRepeatOptions) {
	return new RepeatDirective(itemsBinding, typeof templateOrTemplateBinding === "function" ? templateOrTemplateBinding : () => templateOrTemplateBinding, Object.assign(Object.assign({}, defaultRepeatOptions), options));
}
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/templating/node-observation.js
/**
...
function elements(selector) {
	if (selector) return function(value, index, array) {
...
var NodeObservationBehavior = class {
	...
	*/
	bind(source) {
		...
		if (this.shouldUpdate) this.observe();
	}
	/**
	* Unbinds this behavior from the source.
	* @param source - The source to unbind from.
	*/
	unbind() {
		...
		this.source = null;
		if (this.shouldUpdate) this.disconnect();
	}
	/** @internal */
	handleEvent() {
		this.updateTarget(this.computeNodes());
	...
	computeNodes() {
		...
		if (this.options.filter !== void 0) nodes = nodes.filter(this.options.filter);
		return nodes;
	}
	updateTarget(value) {
...
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/templating/slotted.js
/**
...
function slotted(propertyOrOptions) {
	if (typeof propertyOrOptions === "string") propertyOrOptions = { property: propertyOrOptions };
	return new AttachedBehaviorHTMLDirective("fast-slotted", SlottedBehavior, propertyOrOptions);
}
//#endregion
//#region node_modules/@microsoft/fast-element/dist/esm/templating/children.js
/**
...
var ChildrenBehavior = class extends NodeObservationBehavior {
	...
	*/
	observe() {
		if (this.observer === null) this.observer = new MutationObserver(this.handleEvent.bind(this));
	...
	*/
	getNodes() {
		if ("subtree" in this.options) return Array.from(this.target.querySelectorAll(this.options.selector));
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/patterns/start-end.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/accordion-item/accordion-item.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/node_modules/tslib/tslib.es6.js
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/di/di.js
/**
...
if (!("metadata" in Reflect)) {
	...
	Reflect.defineMetadata = function(key, value, target) {
		...
		if (metadata === void 0) metadataByTarget.set(target, metadata = /* @__PURE__ */ new Map());
		metadata.set(key, value);
	};
	Reflect.getOwnMetadata = function(key, target) {
		const metadata = metadataByTarget.get(target);
		if (metadata !== void 0) return metadata.get(key);
	};
}
/**
...
function cloneArrayWithPossibleProps(source) {
	...
	for (let i = 0; i < len; ++i) {
		key = keys[i];
		if (!isArrayIndex(key)) clone[key] = source[key];
	}
	return clone;
}
/**
...
var DI = Object.freeze({
	...
	*/
	findResponsibleContainer(node) {
		...
		if (owned && owned.responsibleForOwnerRequests) return owned;
	...
	*/
	getOrCreateDOMContainer(node, config) {
		if (!node) return rootDOMContainer || (rootDOMContainer = new ContainerImpl(null, Object.assign({}, ContainerConfiguration.default, config, { parentLocator: () => null })));
		return node.$$container$$ || new ContainerImpl(node, Object.assign({}, ContainerConfiguration.default, config, { parentLocator: DI.findParentContainer }));
	},
	/**
	...
	*/
	getOrCreateAnnotationParamTypes(Type) {
		...
		if (annotationParamtypes === void 0) Reflect.defineMetadata("di:paramtypes", annotationParamtypes = [], Type);
	...
	getDependencies(Type) {
		...
		if (dependencies === void 0) {
			...
			if (inject === void 0) {
				...
				if (designParamtypes === void 0) if (annotationParamtypes === void 0) {
					const Proto = Object.getPrototypeOf(Type);
					if (typeof Proto === "function" && Proto !== Function.prototype) dependencies = cloneArrayWithPossibleProps(DI.getDependencies(Proto));
					else dependencies = [];
				} else dependencies = cloneArrayWithPossibleProps(annotationParamtypes);
				else if (annotationParamtypes === void 0) dependencies = cloneArrayWithPossibleProps(designParamtypes);
				else {
					...
					for (let i = 0; i < len; ++i) {
						...
						if (auAnnotationParamtype !== void 0) dependencies[i] = auAnnotationParamtype;
					}
					...
					for (let i = 0; i < len; ++i) {
						key = keys[i];
						if (!isArrayIndex(key)) dependencies[key] = annotationParamtypes[key];
					}
				}
			} else dependencies = cloneArrayWithPossibleProps(inject);
	...
	*/
	defineProperty(target, propertyName, key, respectConnection = false) {
		...
		Reflect.defineProperty(target, propertyName, { get: function() {
			let value = this[diPropertyKey];
			if (value === void 0) {
				value = (this instanceof HTMLElement ? DI.findResponsibleContainer(this) : DI.getOrCreateDOMContainer()).get(key);
				this[diPropertyKey] = value;
				if (respectConnection && this instanceof FASTElement) {
					const notifier = this.$fastController;
					const handleChange = () => {
						if (DI.findResponsibleContainer(this).get(key) !== this[diPropertyKey]) {
							this[diPropertyKey] = value;
	...
	*/
	createInterface(nameConfigOrCallback, configuror) {
		...
		const Interface = function(target, property, index) {
			if (target == null || new.target !== void 0) throw new Error(`No registration for interface: '${Interface.friendlyName}'`);
			if (property) DI.defineProperty(target, property, Interface, respectConnection);
			else {
		...
		if (configure != null) Interface.register = function(container, key) {
	...
	inject(...dependencies) {
		return function(target, key, descriptor) {
			if (typeof descriptor === "number") {
				...
				if (dep !== void 0) annotationParamtypes[descriptor] = dep;
			} else if (key) DI.defineProperty(target, key, dependencies[0]);
			else {
				...
				for (let i = 0; i < dependencies.length; ++i) {
					dep = dependencies[i];
					if (dep !== void 0) annotationParamtypes[i] = dep;
				}
			}
		};
	},
	...
	*/
	transient(target) {
		target.register = function register(container) {
			return Registration.transient(target, target).register(container);
		};
	...
	*/
	singleton(target, options = defaultSingletonOptions) {
		target.register = function register(container) {
			return Registration.singleton(target, target).register(container);
		};
...
createAllResolver((key, handler, requestor, searchAncestors) => requestor.getAll(key, searchAncestors));
createResolver((key, handler, requestor) => {
	return () => requestor.get(key);
});
createResolver((key, handler, requestor) => {
	if (requestor.has(key, true)) return requestor.get(key);
	else return;
});
/**
...
createResolver((key, handler, _requestor) => createNewInstance(key, handler));
...
var ResolverImpl = class {
	...
	resolve(handler, requestor) {
		switch (this.strategy) {
			...
			case 2: {
				...
				if (factory === null) throw new Error(`Resolver for ${String(this.key)} returned a null factory`);
				return factory.construct(requestor);
			}
			case 3: return this.state(handler, requestor, this);
			case 4: return this.state[0].resolve(handler, requestor);
			case 5: return requestor.get(this.state);
			default: throw new Error(`Invalid resolver strategy specified: ${this.strategy}.`);
		}
	}
	getFactory(container) {
		...
		switch (this.strategy) {
			case 1:
			case 2: return container.getFactory(this.state);
			case 5: return (_c = (_b = (_a = container.getResolver(this.state)) === null || _a === void 0 ? void 0 : _a.getFactory) === null || _b === void 0 ? void 0 : _b.call(_a, container)) !== null && _c !== void 0 ? _c : null;
			default: return null;
		}
	}
};
...
var FactoryImpl = class {
	...
	construct(container, dynamicDependencies) {
		let instance;
		if (dynamicDependencies === void 0) instance = new this.Type(...this.dependencies.map(containerGetKey, container));
		else instance = new this.Type(...this.dependencies.map(containerGetKey, container), ...dynamicDependencies);
		if (this.transformers == null) return instance;
...
var InstrinsicTypeNames = new Set([
	"Array",
	"ArrayBuffer",
	"Boolean",
	"DataView",
...
* @internal
*/
var ContainerImpl = class ContainerImpl {
	constructor(owner, config) {
		...
		this.context = null;
		if (owner !== null) owner.$$container$$ = this;
		this.resolvers = /* @__PURE__ */ new Map();
		this.resolvers.set(Container, containerResolver);
		if (owner instanceof Node) owner.addEventListener(DILocateParentEventType, (e) => {
	...
	get parent() {
		if (this._parent === void 0) this._parent = this.config.parentLocator(this.owner);
		return this._parent;
	}
	get depth() {
	...
	register(...params) {
		if (++this.registerDepth === 100) throw new Error("Unable to autoregister dependency");
		let current;
		let keys;
		let value;
		let j;
		...
		for (let i = 0, ii = params.length; i < ii; ++i) {
			...
			if (isRegistry(current)) current.register(this, context);
			else if (isClass(current)) Registration.singleton(current, current).register(this);
			else {
				...
				for (; j < jj; ++j) {
					...
					if (isRegistry(value)) value.register(this, context);
					else this.register(value);
				}
			}
		}
		--this.registerDepth;
	...
	registerResolver(key, resolver) {
		...
		if (result == null) resolvers.set(key, resolver);
		else if (result instanceof ResolverImpl && result.strategy === 4) result.state.push(resolver);
		else resolvers.set(key, new ResolverImpl(key, 4, [result, resolver]));
		return resolver;
	}
	...
	getResolver(key, autoRegister = true) {
		validateKey(key);
		if (key.resolve !== void 0) return key;
		let current = this;
		let resolver;
		while (current != null) {
	...
	get(key) {
		validateKey(key);
		if (key.$isResolver) return key.resolve(this, this);
		let current = this;
		let resolver;
		while (current != null) {
			resolver = current.resolvers.get(key);
			if (resolver == null) {
				...
				current = current.parent;
			} else return resolver.resolve(current, this);
		}
	...
	getAll(key, searchAncestors = false) {
		...
		if (searchAncestors) {
			...
			while (current != null) {
				resolver = current.resolvers.get(key);
				if (resolver != null) resolutions = resolutions.concat(buildAllResponse(resolver, current, requestor));
				current = current.parent;
			}
			return resolutions;
			...
			if (resolver == null) {
				current = current.parent;
				if (current == null) return emptyArray;
			} else return buildAllResponse(resolver, current, requestor);
		}
		return emptyArray;
	}
	getFactory(Type) {
		...
		if (factory === void 0) {
			if (isNativeFunction(Type)) throw new Error(`${Type.name} is a native function and therefore cannot be safely constructed by DI. If this is intentional, please use a callback or cachedCallback resolver.`);
	...
	jitRegister(keyAsValue, handler) {
		if (typeof keyAsValue !== "function") throw new Error(`Attempted to jitRegister something that is not a constructor: '${keyAsValue}'. Did you forget to register this dependency?`);
		if (InstrinsicTypeNames.has(keyAsValue.name)) throw new Error(`Attempted to jitRegister an intrinsic type: ${keyAsValue.name}. Did you forget to add @inject(Key)`);
		if (isRegistry(keyAsValue)) {
			const registrationResolver = keyAsValue.register(handler);
			if (!(registrationResolver instanceof Object) || registrationResolver.resolve == null) {
				const newResolver = handler.resolvers.get(keyAsValue);
				if (newResolver != void 0) return newResolver;
				throw new Error("A valid resolver was not returned from the static register method");
			}
			return registrationResolver;
		} else if (keyAsValue.$isInterface) throw new Error(`Attempted to jitRegister an interface: ${keyAsValue.friendlyName}`);
		else {
...
function validateKey(key) {
	if (key === null || key === void 0) throw new Error("key/value cannot be null or undefined. Are you trying to inject/register something that doesn't exist with DI?");
}
function buildAllResponse(resolver, handler, requestor) {
	if (resolver instanceof ResolverImpl && resolver.strategy === 4) {
		...
		while (i--) results[i] = state[i].resolve(handler, requestor);
		return results;
	}
...
function isArrayIndex(value) {
	switch (typeof value) {
		case "number": return value >= 0 && (value | 0) === value;
		case "string": {
			const result = isNumericLookup[value];
			if (result !== void 0) return result;
			const length = value.length;
			if (length === 0) return isNumericLookup[value] = false;
			let ch = 0;
			for (let i = 0; i < length; ++i) {
				ch = value.charCodeAt(i);
				if (i === 0 && ch === 48 && length > 1 || ch < 48 || ch > 57) return isNumericLookup[value] = false;
			}
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/design-system/component-presentation.js
...
var ComponentPresentation = Object.freeze({
	...
	*/
	define(tagName, presentation, container) {
		...
		if (presentationRegistry.get(key) === void 0) presentationRegistry.set(key, presentation);
		else presentationRegistry.set(key, false);
	...
	*/
	forTag(tagName, element) {
		...
		if (existing === false) return DI.findResponsibleContainer(element).get(key);
...
var DefaultComponentPresentation = class {
	...
	*/
	applyTo(element) {
		const controller = element.$fastController;
		if (controller.template === null) controller.template = this.template;
		if (controller.styles === null) controller.styles = this.styles;
	}
};
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/foundation-element/foundation-element.js
/**
...
* @public
*/
var FoundationElement = class FoundationElement extends FASTElement {
	constructor() {
		super(...arguments);
	...
	*/
	get $presentation() {
		if (this._presentation === void 0) this._presentation = ComponentPresentation.forTag(this.tagName, this);
		return this._presentation;
	}
	templateChanged() {
		if (this.template !== void 0) this.$fastController.template = this.template;
	}
	stylesChanged() {
		if (this.styles !== void 0) this.$fastController.styles = this.styles;
	}
	/**
	...
	*/
	connectedCallback() {
		if (this.$presentation !== null) this.$presentation.applyTo(this);
...
function resolveOption(option, context, definition) {
	if (typeof option === "function") return option(context, definition);
	return option;
}
/**
...
var FoundationElementRegistry = class {
	...
	register(container, context) {
		...
		const name = `${definition.prefix || context.elementPrefix}-${definition.baseName}`;
		context.tryDefineElement({
			...
			callback: (x) => {
				...
				if (x.shadowRootMode) {
					if (shadowOptions) {
						if (!overrideDefinition.shadowOptions) shadowOptions.mode = x.shadowRootMode;
					} else if (shadowOptions !== null) shadowOptions = { mode: x.shadowRootMode };
				}
				x.defineElement({
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/utilities/apply-mixins.js
/**
...
function applyMixins(derivedCtor, ...baseCtors) {
	...
	baseCtors.forEach((baseCtor) => {
		...
		AttributeConfiguration.locate(baseCtor).forEach((x) => derivedAttributes.push(x));
	});
}
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/accordion-item/accordion-item.js
/**
...
__decorate$1([attr({ mode: "boolean" })], AccordionItem.prototype, "expanded", void 0);
__decorate$1([attr], AccordionItem.prototype, "id", void 0);
applyMixins(AccordionItem, StartEnd);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/accordion/accordion.template.js
/**
* The template for the {@link @microsoft/fast-foundation#Accordion} component.
* @public
*/
var accordionTemplate = (context, definition) => html`
    <template>
        <slot ${slotted({
...
`;
//#endregion
//#region node_modules/@microsoft/fast-web-utilities/dist/aria.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-web-utilities/dist/array.js
/**
...
function findLastIndex(array, predicate) {
	let k = array.length;
	while (k--) if (predicate(array[k], k, array)) return k;
	return -1;
}
//#endregion
//#region node_modules/exenv-es6/dist/can-use-dom.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-web-utilities/dist/dom.js
/**
...
function getDisplayedNodes(rootNode, selector) {
	if (!rootNode || !selector || !isHTMLElement(rootNode)) return;
	return Array.from(rootNode.querySelectorAll(selector)).filter((node) => node.offsetParent !== null);
}
/**
...
function canUseFocusVisible() {
	if (typeof _canUseFocusVisible === "boolean") return _canUseFocusVisible;
	if (!canUseDOM()) {
	...
	if (styleNonce !== null) styleElement.setAttribute("nonce", styleNonce);
...
//#endregion
//#region node_modules/@microsoft/fast-web-utilities/dist/events.js
...
//#endregion
//#region node_modules/@microsoft/fast-web-utilities/dist/key-codes.js
/**
* Key Code values
...
//#endregion
//#region node_modules/@microsoft/fast-web-utilities/dist/localization.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-web-utilities/dist/numbers.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-web-utilities/dist/strings.js
...
//#endregion
//#region node_modules/@microsoft/fast-web-utilities/dist/system-colors.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/accordion/accordion.js
/**
...
var Accordion = class extends FoundationElement {
		...
		this.setItems = () => {
			...
			this.accordionItems.forEach((item, index) => {
				if (item instanceof AccordionItem) {
					...
					if (this.isSingleExpandMode()) this.activeItemIndex !== index ? item.expanded = false : item.expanded = true;
				}
			...
			if (this.isSingleExpandMode()) ((_a = this.findExpandedItem()) !== null && _a !== void 0 ? _a : this.accordionItems[0]).setAttribute("aria-disabled", "true");
		};
		...
		this.activeItemChange = (event) => {
			...
			if (this.isSingleExpandMode()) {
				...
				this.accordionItems.forEach((item) => {
					if (!item.hasAttribute("disabled") && item.id !== this.activeid) item.removeAttribute("aria-disabled");
				});
			}
	...
	*/
	...
	findExpandedItem() {
		for (let item = 0; item < this.accordionItems.length; item++) if (this.accordionItems[item].getAttribute("expanded") === "true") return this.accordionItems[item];
		return null;
	}
	resetItems() {
		this.accordionItems.forEach((item, index) => {
	...
	focusItem() {
		...
		if (element instanceof AccordionItem) element.expandbutton.focus();
	}
};
__decorate$1([attr({ attribute: "expand-mode" })], Accordion.prototype, "expandmode", void 0);
__decorate$1([observable], Accordion.prototype, "accordionItems", void 0);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/anchor/anchor.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/patterns/aria-global.js
/**
...
__decorate$1([attr({ attribute: "aria-atomic" })], ARIAGlobalStatesAndProperties.prototype, "ariaAtomic", void 0);
__decorate$1([attr({ attribute: "aria-busy" })], ARIAGlobalStatesAndProperties.prototype, "ariaBusy", void 0);
__decorate$1([attr({ attribute: "aria-controls" })], ARIAGlobalStatesAndProperties.prototype, "ariaControls", void 0);
__decorate$1([attr({ attribute: "aria-current" })], ARIAGlobalStatesAndProperties.prototype, "ariaCurrent", void 0);
__decorate$1([attr({ attribute: "aria-describedby" })], ARIAGlobalStatesAndProperties.prototype, "ariaDescribedby", void 0);
__decorate$1([attr({ attribute: "aria-details" })], ARIAGlobalStatesAndProperties.prototype, "ariaDetails", void 0);
__decorate$1([attr({ attribute: "aria-disabled" })], ARIAGlobalStatesAndProperties.prototype, "ariaDisabled", void 0);
__decorate$1([attr({ attribute: "aria-errormessage" })], ARIAGlobalStatesAndProperties.prototype, "ariaErrormessage", void 0);
__decorate$1([attr({ attribute: "aria-flowto" })], ARIAGlobalStatesAndProperties.prototype, "ariaFlowto", void 0);
__decorate$1([attr({ attribute: "aria-haspopup" })], ARIAGlobalStatesAndProperties.prototype, "ariaHaspopup", void 0);
__decorate$1([attr({ attribute: "aria-hidden" })], ARIAGlobalStatesAndProperties.prototype, "ariaHidden", void 0);
__decorate$1([attr({ attribute: "aria-invalid" })], ARIAGlobalStatesAndProperties.prototype, "ariaInvalid", void 0);
__decorate$1([attr({ attribute: "aria-keyshortcuts" })], ARIAGlobalStatesAndProperties.prototype, "ariaKeyshortcuts", void 0);
__decorate$1([attr({ attribute: "aria-label" })], ARIAGlobalStatesAndProperties.prototype, "ariaLabel", void 0);
__decorate$1([attr({ attribute: "aria-labelledby" })], ARIAGlobalStatesAndProperties.prototype, "ariaLabelledby", void 0);
__decorate$1([attr({ attribute: "aria-live" })], ARIAGlobalStatesAndProperties.prototype, "ariaLive", void 0);
__decorate$1([attr({ attribute: "aria-owns" })], ARIAGlobalStatesAndProperties.prototype, "ariaOwns", void 0);
__decorate$1([attr({ attribute: "aria-relevant" })], ARIAGlobalStatesAndProperties.prototype, "ariaRelevant", void 0);
__decorate$1([attr({ attribute: "aria-roledescription" })], ARIAGlobalStatesAndProperties.prototype, "ariaRoledescription", void 0);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/anchor/anchor.js
/**
...
* @public
*/
var Anchor$1 = class extends FoundationElement {
	constructor() {
		super(...arguments);
		/**
		...
		*/
		this.handleUnsupportedDelegatesFocus = () => {
			var _a;
			if (window.ShadowRoot && !window.ShadowRoot.prototype.hasOwnProperty("delegatesFocus") && ((_a = this.$fastController.definition.shadowOptions) === null || _a === void 0 ? void 0 : _a.delegatesFocus)) this.focus = () => {
				var _a;
				(_a = this.control) === null || _a === void 0 || _a.focus();
			};
		};
	}
	/**
...
__decorate$1([attr({ attribute: "aria-expanded" })], DelegatesARIALink.prototype, "ariaExpanded", void 0);
applyMixins(DelegatesARIALink, ARIAGlobalStatesAndProperties);
applyMixins(Anchor$1, StartEnd, DelegatesARIALink);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/anchored-region/anchored-region.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/utilities/direction.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/utilities/intersection-service.js
/**
	...
	constructor() {
		...
		*/
		this.requestPosition = (target, callback) => {
			var _a;
			if (this.intersectionDetector === null) return;
			if (this.observedElements.has(target)) {
				(_a = this.observedElements.get(target)) === null || _a === void 0 || _a.push(callback);
				return;
			}
		...
		*/
		this.handleIntersection = (entries) => {
			...
			entries.forEach((entry) => {
				var _a;
				(_a = this.intersectionDetector) === null || _a === void 0 || _a.unobserve(entry.target);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/anchored-region/anchored-region.js
/**
...
* @public
*/
var AnchoredRegion = class AnchoredRegion extends FoundationElement {
	constructor() {
		super(...arguments);
		/**
		...
		*/
		this.update = () => {
			if (!this.pendingPositioningUpdate) this.requestPositionUpdates();
		};
		/**
		* starts observers
		*/
		...
		*/
		this.requestPositionUpdates = () => {
			if (this.anchorElement === null || this.pendingPositioningUpdate) return;
			...
			if (this.viewportElement !== null) AnchoredRegion.intersectionService.requestPosition(this.viewportElement, this.handleIntersection);
		...
		*/
		this.stopObservers = () => {
			if (this.pendingPositioningUpdate) {
				...
				if (this.anchorElement !== null) AnchoredRegion.intersectionService.cancelRequestPosition(this.anchorElement, this.handleIntersection);
				if (this.viewportElement !== null) AnchoredRegion.intersectionService.cancelRequestPosition(this.viewportElement, this.handleIntersection);
			}
			if (this.resizeDetector !== null) this.resizeDetector.disconnect();
		};
		/**
		* Gets the viewport element by id, or defaults to document root
		*/
		this.getViewport = () => {
			if (typeof this.viewport !== "string" || this.viewport === "") return document.documentElement;
		...
		*/
		this.handleIntersection = (entries) => {
			if (!this.pendingPositioningUpdate) return;
			this.pendingPositioningUpdate = false;
			if (!this.applyIntersectionEntries(entries)) return;
			this.updateLayout();
		};
		/**
		...
		*/
		this.applyIntersectionEntries = (entries) => {
			...
			if (regionEntry === void 0 || viewportEntry === void 0 || anchorEntry === void 0) return false;
		...
		*/
		this.isRectDifferent = (rectA, rectB) => {
			if (Math.abs(rectA.top - rectB.top) > this.updateThreshold || Math.abs(rectA.right - rectB.right) > this.updateThreshold || Math.abs(rectA.bottom - rectB.bottom) > this.updateThreshold || Math.abs(rectA.left - rectB.left) > this.updateThreshold) return true;
			return false;
		};
		/**
		...
		*/
		this.reset = () => {
			...
			this.pendingReset = false;
			if (this.anchorElement === null) this.anchorElement = this.getAnchor();
			if (this.viewportElement === null) this.viewportElement = this.getViewport();
		...
		*/
		this.updateLayout = () => {
			...
			if (this.horizontalPositioningMode !== "uncontrolled") {
				...
				if (this.horizontalDefaultPosition === "center") desiredHorizontalPosition = "center";
					...
					if (dirCorrectedHorizontalDefaultPosition === "start" || dirCorrectedHorizontalDefaultPosition === "end") {
						...
						if (this.currentDirection === Direction.ltr) dirCorrectedHorizontalDefaultPosition = dirCorrectedHorizontalDefaultPosition === "start" ? "left" : "right";
						else dirCorrectedHorizontalDefaultPosition = dirCorrectedHorizontalDefaultPosition === "start" ? "right" : "left";
					}
			...
			if (positionChanged) this.$emit("positionchange", this, { bubbles: false });
		};
		/**
		...
		*/
		this.setHorizontalPosition = (desiredHorizontalPosition, nextPositionerDimension) => {
			if (desiredHorizontalPosition === void 0 || this.regionRect === void 0 || this.anchorRect === void 0 || this.viewportRect === void 0) return;
			...
			switch (desiredHorizontalPosition) {
				case "start":
					...
					if (this.horizontalViewportLock && this.anchorRect.left > this.viewportRect.right) this.translateX = this.translateX - (this.anchorRect.left - this.viewportRect.right);
					break;
				case "insetStart":
					this.translateX = this.baseHorizontalOffset - nextRegionWidth + this.anchorRect.width;
					if (this.horizontalViewportLock && this.anchorRect.right > this.viewportRect.right) this.translateX = this.translateX - (this.anchorRect.right - this.viewportRect.right);
					break;
				case "insetEnd":
					this.translateX = this.baseHorizontalOffset;
					if (this.horizontalViewportLock && this.anchorRect.left < this.viewportRect.left) this.translateX = this.translateX - (this.anchorRect.left - this.viewportRect.left);
					break;
				case "end":
					this.translateX = this.baseHorizontalOffset + this.anchorRect.width;
					if (this.horizontalViewportLock && this.anchorRect.right < this.viewportRect.left) this.translateX = this.translateX - (this.anchorRect.right - this.viewportRect.left);
					break;
				case "center":
					...
					if (this.horizontalViewportLock) {
						...
						if (regionLeft < this.viewportRect.left && !(regionRight > this.viewportRect.right)) this.translateX = this.translateX - (regionLeft - this.viewportRect.left);
						else if (regionRight > this.viewportRect.right && !(regionLeft < this.viewportRect.left)) this.translateX = this.translateX - (regionRight - this.viewportRect.right);
					}
					break;
			}
		...
		*/
		this.setVerticalPosition = (desiredVerticalPosition, nextPositionerDimension) => {
			if (desiredVerticalPosition === void 0 || this.regionRect === void 0 || this.anchorRect === void 0 || this.viewportRect === void 0) return;
			...
			switch (desiredVerticalPosition) {
				case "start":
					...
					if (this.verticalViewportLock && this.anchorRect.top > this.viewportRect.bottom) this.translateY = this.translateY - (this.anchorRect.top - this.viewportRect.bottom);
					break;
				case "insetStart":
					this.translateY = this.baseVerticalOffset - nextRegionHeight + this.anchorRect.height;
					if (this.verticalViewportLock && this.anchorRect.bottom > this.viewportRect.bottom) this.translateY = this.translateY - (this.anchorRect.bottom - this.viewportRect.bottom);
					break;
				case "insetEnd":
					this.translateY = this.baseVerticalOffset;
					if (this.verticalViewportLock && this.anchorRect.top < this.viewportRect.top) this.translateY = this.translateY - (this.anchorRect.top - this.viewportRect.top);
					break;
				case "end":
					this.translateY = this.baseVerticalOffset + this.anchorRect.height;
					if (this.verticalViewportLock && this.anchorRect.bottom < this.viewportRect.top) this.translateY = this.translateY - (this.anchorRect.bottom - this.viewportRect.top);
					break;
				case "center":
					...
					if (this.verticalViewportLock) {
						...
						if (regionTop < this.viewportRect.top && !(regionBottom > this.viewportRect.bottom)) this.translateY = this.translateY - (regionTop - this.viewportRect.top);
						else if (regionBottom > this.viewportRect.bottom && !(regionTop < this.viewportRect.top)) this.translateY = this.translateY - (regionBottom - this.viewportRect.bottom);
					}
			}
		...
		*/
		this.getPositioningOptions = (inset) => {
			if (inset) return ["insetStart", "insetEnd"];
			return ["start", "end"];
		};
		/**
		...
		*/
		this.getAvailableSpace = (positionOption, anchorStart, anchorEnd, anchorSpan, viewportStart, viewportEnd) => {
			...
			switch (positionOption) {
				case "start": return spaceStart;
				case "insetStart": return spaceStart + anchorSpan;
				case "insetEnd": return spaceEnd + anchorSpan;
				case "end": return spaceEnd;
				case "center": return Math.min(spaceStart, spaceEnd) * 2 + anchorSpan;
			}
		};
		/**
		...
		*/
		this.startAutoUpdateEventListeners = () => {
			...
			if (this.resizeDetector !== null && this.viewportElement !== null) this.resizeDetector.observe(this.viewportElement);
		};
		/**
		...
		*/
		this.stopAutoUpdateEventListeners = () => {
			...
			if (this.resizeDetector !== null && this.viewportElement !== null) this.resizeDetector.unobserve(this.viewportElement);
		};
	}
	anchorChanged() {
		if (this.initialLayoutComplete) this.anchorElement = this.getAnchor();
	}
	viewportChanged() {
		if (this.initialLayoutComplete) this.viewportElement = this.getViewport();
	}
	...
	fixedPlacementChanged() {
		if (this.$fastController.isConnected && this.initialLayoutComplete) this.initialize();
	}
	autoUpdateModeChanged(prevMode, newMode) {
		if (this.$fastController.isConnected && this.initialLayoutComplete) {
			if (prevMode === "auto") this.stopAutoUpdateEventListeners();
			if (newMode === "auto") this.startAutoUpdateEventListeners();
		}
	}
	...
	*/
	connectedCallback() {
		super.connectedCallback();
		if (this.autoUpdateMode === "auto") this.startAutoUpdateEventListeners();
		this.initialize();
	}
	/**
	* @internal
	*/
	disconnectedCallback() {
		...
		if (this.autoUpdateMode === "auto") this.stopAutoUpdateEventListeners();
		this.stopObservers();
...
__decorate$1([attr({ attribute: "horizontal-positioning-mode" })], AnchoredRegion.prototype, "horizontalPositioningMode", void 0);
__decorate$1([attr({ attribute: "horizontal-default-position" })], AnchoredRegion.prototype, "horizontalDefaultPosition", void 0);
__decorate$1([attr({
...
__decorate$1([attr({ attribute: "horizontal-threshold" })], AnchoredRegion.prototype, "horizontalThreshold", void 0);
__decorate$1([attr({ attribute: "horizontal-scaling" })], AnchoredRegion.prototype, "horizontalScaling", void 0);
__decorate$1([attr({ attribute: "vertical-positioning-mode" })], AnchoredRegion.prototype, "verticalPositioningMode", void 0);
__decorate$1([attr({ attribute: "vertical-default-position" })], AnchoredRegion.prototype, "verticalDefaultPosition", void 0);
__decorate$1([attr({
...
__decorate$1([attr({ attribute: "vertical-threshold" })], AnchoredRegion.prototype, "verticalThreshold", void 0);
__decorate$1([attr({ attribute: "vertical-scaling" })], AnchoredRegion.prototype, "verticalScaling", void 0);
__decorate$1([attr({
	attribute: "fixed-placement",
	mode: "boolean"
})], AnchoredRegion.prototype, "fixedPlacement", void 0);
__decorate$1([attr({ attribute: "auto-update-mode" })], AnchoredRegion.prototype, "autoUpdateMode", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/badge/badge.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/badge/badge.js
/**
...
* @public
*/
var Badge$1 = class extends FoundationElement {
	constructor() {
		super(...arguments);
		this.generateBadgeStyle = () => {
			...
			if (this.fill && !this.color) return fill;
			else if (this.color && !this.fill) return color;
			else return `${color} ${fill}`;
		};
	}
};
__decorate$1([attr({ attribute: "fill" })], Badge$1.prototype, "fill", void 0);
__decorate$1([attr({ attribute: "color" })], Badge$1.prototype, "color", void 0);
__decorate$1([attr({ mode: "boolean" })], Badge$1.prototype, "circular", void 0);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/breadcrumb-item/breadcrumb-item.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/breadcrumb-item/breadcrumb-item.js
/**
* A Breadcrumb Item Custom HTML Element.
*
* @public
*/
var BreadcrumbItem = class extends Anchor$1 {
	constructor() {
		super(...arguments);
		/**
		* @internal
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/breadcrumb/breadcrumb.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/breadcrumb/breadcrumb.js
/**
...
var Breadcrumb = class extends FoundationElement {
	slottedBreadcrumbItemsChanged() {
		if (this.$fastController.isConnected) {
			if (this.slottedBreadcrumbItems === void 0 || this.slottedBreadcrumbItems.length === 0) return;
	...
	*/
	findChildWithHref(node) {
		...
		else if ((_a = node.shadowRoot) === null || _a === void 0 ? void 0 : _a.childElementCount) return (_b = node.shadowRoot) === null || _b === void 0 ? void 0 : _b.querySelector("a[href]");
		else return null;
	}
	/**
	...
	*/
	setAriaCurrent(item, isLastNode) {
		...
		if (childNodeWithHref === null && item.hasAttribute("href") && item instanceof BreadcrumbItem) isLastNode ? item.setAttribute("aria-current", "page") : item.removeAttribute("aria-current");
		else if (childNodeWithHref !== null) isLastNode ? childNodeWithHref.setAttribute("aria-current", "page") : childNodeWithHref.removeAttribute("aria-current");
	}
};
__decorate$1([observable], Breadcrumb.prototype, "slottedBreadcrumbItems", void 0);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/button/button.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/form-associated/form-associated.js
	...
	class D extends C {
		...
		*/
		...
		checkedChanged(prev, next) {
			...
			if (prev !== void 0) this.$emit("change");
			this.validate();
		}
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/button/button.form-associated.js
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/button/button.js
/**
...
* @public
*/
var Button$1 = class extends FormAssociatedButton {
	constructor() {
		super(...arguments);
		/**
		...
		*/
		this.handleClick = (e) => {
			var _a;
			if (this.disabled && ((_a = this.defaultSlottedContent) === null || _a === void 0 ? void 0 : _a.length) <= 1) e.stopPropagation();
		};
		/**
	...
	*/
	connectedCallback() {
		...
		if (elements) elements.forEach((span) => {
...
__decorate$1([attr({ mode: "boolean" })], Button$1.prototype, "autofocus", void 0);
__decorate$1([attr({ attribute: "form" })], Button$1.prototype, "formId", void 0);
...
__decorate$1([attr({ mode: "boolean" })], Button$1.prototype, "formnovalidate", void 0);
...
__decorate$1([attr({ attribute: "aria-expanded" })], DelegatesARIAButton.prototype, "ariaExpanded", void 0);
__decorate$1([attr({ attribute: "aria-pressed" })], DelegatesARIAButton.prototype, "ariaPressed", void 0);
applyMixins(DelegatesARIAButton, ARIAGlobalStatesAndProperties);
applyMixins(Button$1, StartEnd, DelegatesARIAButton);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/calendar/date-formatter.js
/**
	...
	constructor(config) {
		...
		*/
		if (config) for (const key in config) {
			...
			if (key === "date") this.date = this.getDateObject(value);
			else this[key] = value;
		}
	}
	/**
	...
	*/
	getDateObject(date) {
		if (typeof date === "string") {
			...
			if (dates.length < 3) return /* @__PURE__ */ new Date();
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/calendar/calendar.js
/**
* Calendar component
*
...
* @public
*/
var Calendar$1 = class extends FoundationElement {
	constructor() {
		super(...arguments);
		/**
	...
	getDays(info = this.getMonthInfo(), minWeeks = this.minWeeks) {
		...
		while (dayCount < length + 1 || days.length < minWeeks || days[days.length - 1].length % 7 !== 0) {
			...
			const date = {
				...
				month,
				year,
				disabled: this.dateInString(dateString, this.disabledDates),
				selected: this.dateInString(dateString, this.selectedDates)
			};
			const target = days[days.length - 1];
			if (days.length === 0 || target.length % 7 === 0) days.push([date]);
			else target.push(date);
			dayCount++;
		}
		return days;
	}
	...
	*/
	handleKeydown(event, date) {
		if (event.key === "Enter") this.handleDateSelect(event, date);
		return true;
	}
};
__decorate$1([attr({ mode: "boolean" })], Calendar$1.prototype, "readonly", void 0);
__decorate$1([attr], Calendar$1.prototype, "locale", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], Calendar$1.prototype, "month", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], Calendar$1.prototype, "year", void 0);
__decorate$1([attr({
...
__decorate$1([attr({ attribute: "disabled-dates" })], Calendar$1.prototype, "disabledDates", void 0);
__decorate$1([attr({ attribute: "selected-dates" })], Calendar$1.prototype, "selectedDates", void 0);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/data-grid/data-grid.options.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/data-grid/data-grid-row.js
/**
	...
	*/
	connectedCallback() {
		...
		if (this.refocusOnLoad) {
			this.refocusOnLoad = false;
			if (this.cellElements.length > this.focusColumnIndex) this.cellElements[this.focusColumnIndex].focus();
		}
	}
	/**
	* @internal
...
__decorate$1([attr({ attribute: "grid-template-columns" })], DataGridRow.prototype, "gridTemplateColumns", void 0);
__decorate$1([attr({ attribute: "row-type" })], DataGridRow.prototype, "rowType", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/data-grid/data-grid.template.js
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/data-grid/data-grid.js
/**
...
* @public
*/
var DataGrid = class DataGrid extends FoundationElement {
	constructor() {
		super();
		/**
		...
		*/
		...
		this.focusOnCell = (rowIndex, columnIndex, scrollIntoView) => {
			...
			const cells = this.rowElements[focusRowIndex].querySelectorAll("[role=\"cell\"], [role=\"gridcell\"], [role=\"columnheader\"], [role=\"rowheader\"]");
			...
			if (mutations && mutations.length) {
				mutations.forEach((mutation) => {
					mutation.addedNodes.forEach((newNode) => {
						if (newNode.nodeType === 1 && newNode.getAttribute("role") === "row") newNode.columnDefinitions = this.columnDefinitions;
					});
				});
	...
	*/
	...
	noTabbingChanged() {
		if (this.$fastController.isConnected) if (this.noTabbing) this.setAttribute("tabIndex", "-1");
		else this.setAttribute("tabIndex", this.contains(document.activeElement) || this === document.activeElement ? "-1" : "0");
	}
	generateHeaderChanged() {
		if (this.$fastController.isConnected) this.toggleGeneratedHeader();
	}
	gridTemplateColumnsChanged() {
		if (this.$fastController.isConnected) this.updateRowIndexes();
	}
	rowsDataChanged() {
	...
	*/
	connectedCallback() {
		...
		if (this.noTabbing) this.setAttribute("tabindex", "-1");
	...
	*/
	...
	queueFocusUpdate() {
		if (this.isUpdatingFocus && (this.contains(document.activeElement) || this === document.activeElement)) return;
	...
	toggleGeneratedHeader() {
		...
		if (this.generateHeader !== GenerateHeaderOptions.none && this.rowsData.length > 0) {
			...
			if (this.firstChild !== null || this.rowsPlaceholder !== null) this.insertBefore(generatedHeaderElement, this.firstChild !== null ? this.firstChild : this.rowsPlaceholder);
			return;
		}
	}
};
...
__decorate$1([attr({ attribute: "generate-header" })], DataGrid.prototype, "generateHeader", void 0);
__decorate$1([attr({ attribute: "grid-template-columns" })], DataGrid.prototype, "gridTemplateColumns", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/data-grid/data-grid-cell.js
...
__decorate$1([attr({ attribute: "cell-type" })], DataGridCell.prototype, "cellType", void 0);
__decorate$1([attr({ attribute: "grid-column" })], DataGridCell.prototype, "gridColumn", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/data-grid/data-grid-row.template.js
...
var dataGridRowTemplate = (context, definition) => {
	return html`
        <template
            ...
            :defaultCellItemTemplate="${createCellItemTemplate(context)}"
            :defaultHeaderCellItemTemplate="${createHeaderCellItemTemplate(context)}"
            ${children({
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/data-grid/data-grid-cell.template.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/calendar/calendar.template.js
/**
...
var calendarRowTemplate = (context, todayString) => {
	...
	return html`
        ...
        >
        ${repeat((x) => x, calendarCellTemplate(context, todayString), { positioning: true })}
        </${rowTag}>
    `;
};
/**
...
var interactiveCalendarGridTemplate = (context, todayString) => {
    ...
    <${gridTag} class="days interact" part="days" generate-header="none">
        <${rowTag}
            ...
            grid-template-columns="1fr 1fr 1fr 1fr 1fr 1fr 1fr"
        >
            ${repeat((x) => x.getWeekdayText(), calendarWeekdayTemplate(context), { positioning: true })}
        </${rowTag}>
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/card/card.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/card/card.js
/**
...
* @public
*/
var Card$1 = class extends FoundationElement {};
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/checkbox/checkbox.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/checkbox/checkbox.form-associated.js
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/checkbox/checkbox.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/listbox-option/listbox-option.js
/**
...
var ListboxOption = class extends FoundationElement {
	...
	*/
	contentChanged(prev, next) {
		if (this.proxy instanceof HTMLOptionElement) this.proxy.textContent = this.textContent;
	...
	set value(next) {
		...
		this.dirtyValue = true;
		if (this.proxy instanceof HTMLOptionElement) this.proxy.value = newValue;
...
__decorate$1([attr({ mode: "boolean" })], ListboxOption.prototype, "disabled", void 0);
__decorate$1([attr({
	attribute: "selected",
	mode: "boolean"
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/listbox/listbox.js
/**
...
* @public
*/
var Listbox$1 = class Listbox$1 extends FoundationElement {
	constructor() {
		super(...arguments);
		/**
	...
	*/
	handleChange(source, propertyName) {
		switch (propertyName) {
			case "selected":
				if (Listbox$1.slottedOptionFilter(source)) this.selectedIndex = this.options.indexOf(source);
				this.setSelectedOptions();
				break;
		}
	}
	...
	*/
	keydownHandler(e) {
		...
		switch (key) {
			...
			case " ": if (this.typeaheadExpired) return true;
			default:
	...
	*/
	selectedIndexChanged(prev, next) {
		...
		if (((_a = this.options[this.selectedIndex]) === null || _a === void 0 ? void 0 : _a.disabled) && typeof prev === "number") {
			...
			if (next === newNext) this.selectedIndexChanged(next, newNext);
			return;
		}
		this.setSelectedOptions();
	}
	...
	*/
	selectedOptionsChanged(prev, next) {
		...
		(_a = this.options) === null || _a === void 0 || _a.forEach((o) => {
	...
	*/
	selectFirstOption() {
		var _a, _b;
		if (!this.disabled) this.selectedIndex = (_b = (_a = this.options) === null || _a === void 0 ? void 0 : _a.findIndex((o) => !o.disabled)) !== null && _b !== void 0 ? _b : -1;
	}
	/**
	...
	*/
	selectNextOption() {
		if (!this.disabled && this.selectedIndex < this.options.length - 1) this.selectedIndex += 1;
	}
	/**
...
* @internal
*/
Listbox$1.TYPE_AHEAD_TIMEOUT_MS = 1e3;
__decorate$1([attr({ mode: "boolean" })], Listbox$1.prototype, "disabled", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/select/select.options.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/combobox/combobox.form-associated.js
var _Combobox = class extends Listbox$1 {};
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/combobox/combobox.options.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/combobox/combobox.js
/**
...
* @public
*/
var Combobox$1 = class extends FormAssociatedCombobox {
	constructor() {
		super(...arguments);
		/**
	...
	*/
	clickHandler(e) {
		const captured = e.target.closest(`option,[role=option]`);
		if (this.disabled || (captured === null || captured === void 0 ? void 0 : captured.disabled)) return;
		if (this.open) {
			if (e.composedPath()[0] === this.control) return;
			if (captured) {
	...
	*/
	disabledChanged(prev, next) {
		if (super.disabledChanged) super.disabledChanged(prev, next);
		this.ariaDisabled = this.disabled ? "true" : "false";
	}
	/**
	...
	*/
	focusAndScrollOptionIntoView() {
		if (this.contains(document.activeElement)) {
			this.control.focus();
			if (this.firstSelectedOption) requestAnimationFrame(() => {
				var _a;
				(_a = this.firstSelectedOption) === null || _a === void 0 || _a.scrollIntoView({ block: "nearest" });
			});
		}
	}
	/**
	...
	*/
	focusoutHandler(e) {
		...
		if (!this.options || !this.options.includes(focusTarget)) this.open = false;
	}
	/**
	...
	*/
	setDefaultSelectedOption() {
		if (this.$fastController.isConnected && this.options) {
			...
			if (!this.dirtyValue && this.firstSelectedOption) this.value = this.firstSelectedOption.text;
			this.setSelectedOptions();
		}
	}
	/**
	...
	*/
	setPositioning() {
		...
		const availableBottom = window.innerHeight - currentBox.bottom;
...
__decorate$1([attr({ attribute: "position" })], Combobox$1.prototype, "positionAttribute", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/combobox/combobox.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/utilities/composed-parent.js
/**
...
function composedParent(element) {
	...
	if (parentNode) return parentNode;
		...
		if (rootNode.host instanceof HTMLElement) return rootNode.host;
	}
	return null;
}
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/utilities/composed-contains.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/design-token/custom-property-manager.js
...
* @internal
*/
var RootStyleSheetTarget = class RootStyleSheetTarget {
	setProperty(name, value) {
		RootStyleSheetTarget.properties[name] = value;
		for (const target of RootStyleSheetTarget.roots.values()) PropertyTargetManager.getOrCreate(RootStyleSheetTarget.normalizeRoot(target)).setProperty(name, value);
	}
	removeProperty(name) {
		delete RootStyleSheetTarget.properties[name];
		for (const target of RootStyleSheetTarget.roots.values()) PropertyTargetManager.getOrCreate(RootStyleSheetTarget.normalizeRoot(target)).removeProperty(name);
	}
	static registerRoot(root) {
		...
		if (!roots.has(root)) {
			...
			for (const key in RootStyleSheetTarget.properties) target.setProperty(key, RootStyleSheetTarget.properties[key]);
		}
	}
	static unregisterRoot(root) {
		...
		if (roots.has(root)) {
			...
			for (const key in RootStyleSheetTarget.properties) target.removeProperty(key);
		}
	}
	/**
...
* @internal
*/
var PropertyTargetManager = Object.freeze({ getOrCreate(source) {
	if (propertyTargetCache.has(source)) return propertyTargetCache.get(source);
	let target;
	if (source === defaultElement) target = new RootStyleSheetTarget();
	else if (source instanceof Document) target = DOM.supportsAdoptedStyleSheets ? new DocumentStyleSheetTarget() : new HeadStyleElementStyleSheetTarget();
	else if (isFastElement(source)) target = new propertyTargetCtor(source);
	else target = new ElementStyleSheetTarget(source);
	propertyTargetCache.set(source, target);
	return target;
} });
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/design-token/design-token.js
/**
* Implementation of {@link (DesignToken:interface)}
*/
var DesignTokenImpl = class DesignTokenImpl extends CSSDirective {
	...
	*/
	...
	setValueFor(element, value) {
		...
		if (value instanceof DesignTokenImpl) value = this.alias(value);
		DesignTokenNode.getOrCreate(element).set(this, value);
		return this;
	}
	deleteValueFor(element) {
		this._appliedTo.delete(element);
		if (DesignTokenNode.existsFor(element)) DesignTokenNode.getOrCreate(element).delete(this);
		return this;
	}
	withDefault(value) {
	...
	subscribe(subscriber, target) {
		...
		if (target && !DesignTokenNode.existsFor(target)) DesignTokenNode.getOrCreate(target);
		if (!subscriberSet.has(subscriber)) subscriberSet.add(subscriber);
	}
	unsubscribe(subscriber, target) {
		const list = this.subscribers.get(target || this);
		if (list && list.has(subscriber)) list.delete(subscriber);
	}
	/**
	...
	*/
	notify(element) {
		...
		if (this.subscribers.has(this)) this.subscribers.get(this).forEach((sub) => sub.handleChange(record));
		if (this.subscribers.has(element)) this.subscribers.get(element).forEach((sub) => sub.handleChange(record));
	}
	/**
...
*/
var DesignTokenNode = class DesignTokenNode {
	constructor(target) {
		this.target = target;
		/**
		...
		*/
		this.tokenValueChangeHandler = { handleChange: (source, arg) => {
		...
		if (target instanceof FASTElement) target.$fastController.addBehaviors([this]);
		else if (target.isConnected) this.bind();
	}
	/**
	...
	*/
	static findParent(node) {
		if (!(defaultElement === node.target)) {
			...
			while (parent !== null) {
				if (nodeCache.has(parent)) return nodeCache.get(parent);
	...
	updateCSSTokenReflection(source, token) {
		if (DesignTokenImpl.isCSSDesignToken(token)) {
			...
			if (parent) {
				...
				if (parentValue !== sourceValue && !reflecting) this.reflectToCSS(token);
				else if (parentValue === sourceValue && reflecting) this.stopReflectToCSS(token);
			} else if (!reflecting) this.reflectToCSS(token);
		}
	}
	/**
	...
	*/
	getRaw(token) {
		var _a;
		if (this.assignedValues.has(token)) return this.assignedValues.get(token);
	...
	*/
	set(token, value) {
		if (DesignTokenImpl.isDerivedDesignTokenValue(this.assignedValues.get(token))) this.tearDownBindingObserver(token);
		this.assignedValues.set(token, value);
		if (DesignTokenImpl.isDerivedDesignTokenValue(value)) this.setupBindingObserver(token, value);
		else this.store.set(token, value);
	}
	/**
	...
	*/
	delete(token) {
		...
		if (upstream) this.hydrate(token, upstream);
		else this.store.delete(token);
	}
	/**
	...
	*/
	bind() {
		...
		for (const key of this.assignedValues.keys()) key.notify(this.target);
	}
	/**
	* Invoked when the DesignTokenNode.target is detached from the document
	*/
	unbind() {
		if (this.parent) childToParent.get(this).removeChild(this);
		for (const token of this.bindingObservers.keys()) this.tearDownBindingObserver(token);
	}
	/**
	...
	*/
	appendChild(child) {
		if (child.parent) childToParent.get(child).removeChild(child);
	...
	*/
	hydrate(token, value) {
		if (!this.has(token)) {
			...
			if (DesignTokenImpl.isDerivedDesignTokenValue(value)) if (observer) {
				if (observer.source !== value) {
					this.tearDownBindingObserver(token);
					this.setupBindingObserver(token, value);
				}
			} else this.setupBindingObserver(token, value);
			else {
				if (observer) this.tearDownBindingObserver(token);
...
var DesignToken = Object.freeze({
	...
	*/
	notifyConnection(element) {
		if (!element.isConnected || !DesignTokenNode.existsFor(element)) return false;
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/design-system/design-system.js
/**
...
var designSystemKey = DI.createInterface((x) => x.cachedCallback((handler) => {
	if (rootDesignSystem === null) rootDesignSystem = new DefaultDesignSystem(null, handler);
...
var DesignSystem = Object.freeze({
	...
	*/
	responsibleFor(element) {
		...
		return DI.findResponsibleContainer(element).get(designSystemKey);
	},
	/**
	...
	*/
	getOrCreate(node) {
		if (!node) {
			if (rootDesignSystem === null) rootDesignSystem = DI.getOrCreateDOMContainer().get(designSystemKey);
			return rootDesignSystem;
		}
		const owned = node.$$designSystem$$;
		if (owned) return owned;
		const container = DI.getOrCreateDOMContainer(node);
		if (container.has(designSystemKey, false)) return container.get(designSystemKey);
		else {
...
var DefaultDesignSystem = class {
	constructor(owner, container) {
		...
		if (owner !== null) owner.$$designSystem$$ = this;
	}
	withPrefix(prefix) {
		this.prefix = prefix;
		return this;
	...
	register(...registrations) {
		...
		const context = {
			...
			tryDefineElement(params, elementDefinitionType, elementDefinitionCallback) {
				...
				while (typeFoundByName) {
					...
					switch (result) {
						case ElementDisambiguation.ignoreDuplicate: return;
				...
				if (needsDefine) {
					if (elementTagsByType.has(type) || type === FoundationElement) type = class extends type {};
					elementTypesByTag.set(elementName, type);
					elementTagsByType.set(type, elementName);
					if (baseClass) elementTagsByType.set(baseClass, elementName);
				}
		...
		if (!this.designTokensInitialized) {
			...
			if (this.designTokenRoot !== null) DesignToken.registerRoot(this.designTokenRoot);
		}
		container.registerWithContext(context, ...registrations);
		for (const entry of elementDefinitionEntries) {
			entry.callback(entry);
			if (entry.willDefine && entry.definition !== null) entry.definition.define();
		}
		return this;
	}
};
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/dialog/dialog.template.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/dialog/dialog.js
/**
...
* @public
*/
var Dialog = class Dialog extends FoundationElement {
	constructor() {
		super(...arguments);
		/**
		...
		*/
		...
		this.trapFocusChanged = () => {
			if (this.$fastController.isConnected) this.updateTrapFocus();
		};
		/**
		* @internal
		*/
		...
		this.handleDocumentKeydown = (e) => {
			if (!e.defaultPrevented && !this.hidden) switch (e.key) {
				case keyEscape:
					this.dismiss();
					e.preventDefault();
					break;
		...
		this.getTabQueueBounds = () => {
			return Dialog.reduceTabbableItems([], this);
		};
		/**
		...
		*/
		this.focusFirstElement = () => {
			...
			else if (this.dialog instanceof HTMLElement) this.dialog.focus();
		};
		/**
	...
	*/
	static reduceTabbableItems(elements, element) {
		...
		if (element.childElementCount) return elements.concat(Array.from(element.children).reduce(Dialog.reduceTabbableItems, []));
		return elements;
	}
	/**
...
__decorate$1([attr({ mode: "boolean" })], Dialog.prototype, "modal", void 0);
__decorate$1([attr({ mode: "boolean" })], Dialog.prototype, "hidden", void 0);
__decorate$1([attr({
	attribute: "trap-focus",
	mode: "boolean"
})], Dialog.prototype, "trapFocus", void 0);
__decorate$1([attr({ attribute: "aria-describedby" })], Dialog.prototype, "ariaDescribedby", void 0);
__decorate$1([attr({ attribute: "aria-labelledby" })], Dialog.prototype, "ariaLabelledby", void 0);
__decorate$1([attr({ attribute: "aria-label" })], Dialog.prototype, "ariaLabel", void 0);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/divider/divider.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/divider/divider.options.js
/**
* Divider roles
* @public
*/
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/divider/divider.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/flipper/flipper.options.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/flipper/flipper.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/flipper/flipper.js
/**
	...
	*/
	keyupHandler(e) {
		if (!this.hiddenFromAT) {
			const key = e.key;
			if (key === "Enter" || key === "Space") this.$emit("click", e);
			if (key === "Escape") this.blur();
		}
	}
};
__decorate$1([attr({ mode: "boolean" })], Flipper.prototype, "disabled", void 0);
__decorate$1([attr({
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/listbox-option/listbox-option.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/listbox/listbox.element.js
/**
* A Listbox Custom HTML Element.
...
* @public
*/
var ListboxElement = class extends Listbox$1 {
	constructor() {
		super(...arguments);
		/**
	...
	*/
	checkFirstOption(preserveChecked = false) {
		if (preserveChecked) {
			if (this.rangeStartIndex === -1) this.rangeStartIndex = this.activeIndex + 1;
	...
	*/
	clickHandler(e) {
		var _a;
		if (!this.multiple) return super.clickHandler(e);
		const captured = (_a = e.target) === null || _a === void 0 ? void 0 : _a.closest(`[role=option]`);
		if (!captured || captured.disabled) return;
	...
	*/
	keydownHandler(e) {
		if (!this.multiple) return super.keydownHandler(e);
		if (this.disabled) return true;
	...
	*/
	mousedownHandler(e) {
		if (e.offsetX >= 0 && e.offsetX <= this.scrollWidth) return super.mousedownHandler(e);
	}
	/**
	* Switches between single-selection and multi-selection mode.
	*
...
__decorate$1([attr({ mode: "boolean" })], ListboxElement.prototype, "multiple", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], ListboxElement.prototype, "size", void 0);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/listbox/listbox.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/menu-item/menu-item.options.js
/**
* Menu items roles.
* @public
*/
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/menu-item/menu-item.js
/**
* A Switch Custom HTML Element.
...
var MenuItem = class extends FoundationElement {
	constructor() {
		...
		this.handleMenuItemKeyDown = (e) => {
			...
			switch (e.key) {
				...
				case keyArrowRight:
					this.expandAndFocus();
					return false;
				case keyArrowLeft: if (this.expanded) {
					this.expanded = false;
					this.focus();
					return false;
				}
		...
		*/
		this.handleMenuItemClick = (e) => {
			if (e.defaultPrevented || this.disabled) return false;
			this.invoke();
			return false;
		};
		/**
		...
		*/
		this.submenuLoaded = () => {
			if (!this.focusSubmenuOnLoad) return;
			this.focusSubmenuOnLoad = false;
			if (this.hasSubmenu) {
				this.submenu.focus();
				this.setAttribute("tabindex", "-1");
		...
		*/
		this.handleMouseOver = (e) => {
			if (this.disabled || !this.hasSubmenu || this.expanded) return false;
			this.expanded = true;
			return false;
		};
		/**
		...
		*/
		this.handleMouseOut = (e) => {
			if (!this.expanded || this.contains(document.activeElement)) return false;
			this.expanded = false;
			return false;
		};
		/**
		...
		this.invoke = () => {
			...
			switch (this.role) {
				...
				case MenuItemRole.menuitem:
					this.updateSubmenu();
					if (this.hasSubmenu) this.expandAndFocus();
	...
	expandedChanged(oldValue) {
		if (this.$fastController.isConnected) {
			if (this.submenu === void 0) return;
			if (this.expanded === false) this.submenu.collapseExpandedItem();
			else this.currentDirection = getDirection(this);
			this.$emit("expanded-change", this, { bubbles: false });
		}
	}
	checkedChanged(oldValue, newValue) {
		if (this.$fastController.isConnected) this.$emit("change");
	}
	/**
	* @internal
	*/
	connectedCallback() {
		...
		if (!this.startColumnCount) this.startColumnCount = 1;
...
__decorate$1([attr({ mode: "boolean" })], MenuItem.prototype, "disabled", void 0);
__decorate$1([attr({ mode: "boolean" })], MenuItem.prototype, "expanded", void 0);
__decorate$1([observable], MenuItem.prototype, "startColumnCount", void 0);
__decorate$1([attr], MenuItem.prototype, "role", void 0);
__decorate$1([attr({ mode: "boolean" })], MenuItem.prototype, "checked", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/menu-item/menu-item.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/menu/menu.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/menu/menu.js
/**
...
* @public
*/
var Menu$1 = class Menu$1 extends FoundationElement {
	constructor() {
		super(...arguments);
		...
		*/
		...
		this.handleExpandedChanged = (e) => {
			if (e.defaultPrevented || e.target === null || this.menuItems === void 0 || this.menuItems.indexOf(e.target) < 0) return;
			e.preventDefault();
			...
			if (changedItem.expanded) {
				if (this.expandedItem !== null && this.expandedItem !== changedItem) this.expandedItem.expanded = false;
				this.menuItems[this.focusIndex].setAttribute("tabindex", "-1");
		...
		this.removeItemListeners = () => {
			if (this.menuItems !== void 0) this.menuItems.forEach((item) => {
		...
		this.setItems = () => {
			...
			if (menuItems.length) this.focusIndex = 0;
			function elementIndent(el) {
				const role = el.getAttribute("role");
				const startSlot = el.querySelector("[slot=start]");
				if (role !== MenuItemRole.menuitem && startSlot === null) return 1;
				else if (role === MenuItemRole.menuitem && startSlot !== null) return 1;
				else if (role !== MenuItemRole.menuitem && startSlot !== null) return 2;
				else return 0;
			}
			...
			menuItems.forEach((item, index) => {
				...
				item.addEventListener("focus", this.handleItemFocus);
				if (item instanceof MenuItem || "startColumnCount" in item) item.startColumnCount = indent;
			});
		};
		/**
		...
		*/
		this.changeHandler = (e) => {
			if (this.menuItems === void 0) return;
			...
			if (changedMenuItem.role === "menuitemradio" && changedMenuItem.checked === true) {
				for (let i = changeItemIndex - 1; i >= 0; --i) {
					...
					if (role === MenuItemRole.menuitemradio) item.checked = false;
					if (role === "separator") break;
				}
	...
	itemsChanged(oldValue, newValue) {
		if (this.$fastController.isConnected && this.menuItems !== void 0) this.setItems();
	}
	/**
	* @internal
	*/
	...
	*/
	handleMenuKeyDown(e) {
		if (e.defaultPrevented || this.menuItems === void 0) return;
		switch (e.key) {
			case keyArrowDown:
	...
	setFocus(focusIndex, adjustment) {
		...
		while (focusIndex >= 0 && focusIndex < this.menuItems.length) {
			...
			if (this.isFocusableElement(child)) {
				if (this.focusIndex > -1 && this.menuItems.length >= this.focusIndex - 1) this.menuItems[this.focusIndex].setAttribute("tabindex", "-1");
				this.focusIndex = focusIndex;
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/number-field/number-field.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/text-field/text-field.form-associated.js
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/text-field/text-field.options.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/text-field/text-field.js
/**
...
* @public
*/
var TextField$1 = class extends FormAssociatedTextField {
	constructor() {
		super(...arguments);
		/**
	...
	*/
	connectedCallback() {
		...
		this.proxy.setAttribute("type", this.type);
		this.validate();
		if (this.autofocus) DOM.queueUpdate(() => {
			this.focus();
		});
	}
	/**
...
__decorate$1([attr({ mode: "boolean" })], TextField$1.prototype, "autofocus", void 0);
...
__decorate$1([attr({ converter: nullableNumberConverter })], TextField$1.prototype, "maxlength", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], TextField$1.prototype, "minlength", void 0);
__decorate$1([attr], TextField$1.prototype, "pattern", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], TextField$1.prototype, "size", void 0);
__decorate$1([attr({ mode: "boolean" })], TextField$1.prototype, "spellcheck", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/number-field/number-field.form-associated.js
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/number-field/number-field.js
/**
...
* @public
*/
var NumberField$1 = class extends FormAssociatedNumberField {
	constructor() {
		super(...arguments);
		/**
	...
	*/
	maxChanged(previous, next) {
		...
		if (this.min !== void 0 && this.min !== min) this.min = min;
	...
	*/
	minChanged(previous, next) {
		...
		if (this.max !== void 0 && this.max !== max) this.max = max;
		this.value = this.getValidValue(this.value);
	}
	/**
	...
	*/
	valueChanged(previous, next) {
		...
		if (this.control && !this.isUserInput) this.control.value = this.value;
...
__decorate$1([attr({ mode: "boolean" })], NumberField$1.prototype, "autofocus", void 0);
__decorate$1([attr({
	attribute: "hide-step",
	mode: "boolean"
...
__decorate$1([attr({ converter: nullableNumberConverter })], NumberField$1.prototype, "maxlength", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], NumberField$1.prototype, "minlength", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], NumberField$1.prototype, "size", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], NumberField$1.prototype, "step", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], NumberField$1.prototype, "max", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], NumberField$1.prototype, "min", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/progress-ring/progress-ring.template.js
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/progress/base-progress.js
/**
...
var BaseProgress = class extends FoundationElement {
	...
	valueChanged() {
		if (this.$fastController.isConnected) this.updatePercentComplete();
	}
	minChanged() {
		if (this.$fastController.isConnected) this.updatePercentComplete();
	}
...
__decorate$1([attr({ converter: nullableNumberConverter })], BaseProgress.prototype, "value", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], BaseProgress.prototype, "min", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], BaseProgress.prototype, "max", void 0);
__decorate$1([attr({ mode: "boolean" })], BaseProgress.prototype, "paused", void 0);
__decorate$1([observable], BaseProgress.prototype, "percentComplete", void 0);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/progress/progress.template.js
/**
* The template for the {@link @microsoft/fast-foundation#BaseProgress} component.
* @public
*/
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/radio-group/radio-group.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/radio-group/radio-group.js
/**
...
var RadioGroup = class extends FoundationElement {
	constructor() {
		...
		*/
			...
			if (changedRadio.checked) {
				this.slottedRadioButtons.forEach((radio) => {
					if (radio !== changedRadio) {
						radio.checked = false;
						if (!this.isInsideFoundationToolbar) radio.setAttribute("tabindex", "-1");
					}
				});
		...
		this.moveToRadioByIndex = (group, index) => {
			...
			if (!this.isInsideToolbar) {
				...
				if (radio.readOnly) this.slottedRadioButtons.forEach((nextRadio) => {
					if (nextRadio !== radio) nextRadio.setAttribute("tabindex", "-1");
				});
				else {
					radio.checked = true;
		...
		this.moveRightOffGroup = () => {
			var _a;
			(_a = this.nextElementSibling) === null || _a === void 0 || _a.focus();
		};
		this.moveLeftOffGroup = () => {
			var _a;
			(_a = this.previousElementSibling) === null || _a === void 0 || _a.focus();
		};
		/**
		* @internal
		*/
		this.focusOutHandler = (e) => {
			...
			if (focusedIndex === 0 && index === focusedIndex || focusedIndex === group.length - 1 && focusedIndex === index) if (!this.selectedRadio) {
				this.focusedRadio = group[0];
				this.focusedRadio.setAttribute("tabindex", "0");
				group.forEach((nextRadio) => {
					if (nextRadio !== this.focusedRadio) nextRadio.setAttribute("tabindex", "-1");
				});
			} else {
				...
				if (!this.isInsideFoundationToolbar) {
					...
					group.forEach((nextRadio) => {
						if (nextRadio !== this.selectedRadio) nextRadio.setAttribute("tabindex", "-1");
					});
				}
			}
			return true;
		...
		*/
		...
		this.shouldMoveOffGroupToTheRight = (index, group, key) => {
			return index === group.length && this.isInsideToolbar && key === "ArrowRight";
		};
		this.shouldMoveOffGroupToTheLeft = (group, key) => {
			return (this.focusedRadio ? group.indexOf(this.focusedRadio) - 1 : 0) < 0 && this.isInsideToolbar && key === "ArrowLeft";
		};
		...
		this.moveRight = (e) => {
			...
			if (this.shouldMoveOffGroupToTheRight(index, group, e.key)) {
				this.moveRightOffGroup();
				return;
			} else if (index === group.length) index = 0;
			while (index < group.length && group.length > 1) if (!group[index].disabled) {
				this.moveToRadioByIndex(group, index);
				break;
			} else if (this.focusedRadio && index === group.indexOf(this.focusedRadio)) break;
			else if (index + 1 >= group.length) if (this.isInsideToolbar) break;
			else index = 0;
			else index += 1;
		};
		this.moveLeft = (e) => {
			...
			while (index >= 0 && group.length > 1) if (!group[index].disabled) {
				this.moveToRadioByIndex(group, index);
				break;
			} else if (this.focusedRadio && index === group.indexOf(this.focusedRadio)) break;
		...
		*/
		this.keydownHandler = (e) => {
			const key = e.key;
			if (key in ArrowKeys && this.isInsideFoundationToolbar) return true;
			switch (key) {
				case keyEnter:
					this.checkFocusedRadio();
					break;
				case keyArrowRight:
				case keyArrowDown:
					if (this.direction === Direction.ltr) this.moveRight(e);
					else this.moveLeft(e);
					break;
				case keyArrowLeft:
				case keyArrowUp:
					if (this.direction === Direction.ltr) this.moveLeft(e);
					else this.moveRight(e);
					break;
				default: return true;
			}
		};
	...
	readOnlyChanged() {
		if (this.slottedRadioButtons !== void 0) this.slottedRadioButtons.forEach((radio) => {
	...
	nameChanged() {
		if (this.slottedRadioButtons) this.slottedRadioButtons.forEach((radio) => {
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/radio/radio.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/radio/radio.form-associated.js
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/radio/radio.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/horizontal-scroll/horizontal-scroll.js
/**
...
* @public
*/
var HorizontalScroll$1 = class extends FoundationElement {
	constructor() {
		super(...arguments);
		/**
		* @internal
	...
	*/
	scrollItemsChanged(previous, next) {
		if (next && !this.updatingItems) DOM.queueUpdate(() => this.setStops());
	}
	/**
	* destroys the instance's resize observer
	* @internal
	...
	*/
	updateScrollStops() {
		...
		const updatedItems = this.scrollItems.reduce((scrollItems, scrollItem) => {
			if (scrollItem instanceof HTMLSlotElement) return scrollItems.concat(scrollItem.assignedElements());
	...
	*/
	validateStops(reinit = true) {
		...
		if (!hasStops() && reinit) this.setStops();
		return hasStops();
	}
	/**
	*
	...
	*/
	setFlippers() {
		...
		(_a = this.previousFlipperContainer) === null || _a === void 0 || _a.classList.toggle("disabled", position === 0);
		if (this.scrollStops) {
			const lastStop = Math.abs(this.scrollStops[this.scrollStops.length - 1]);
			(_b = this.nextFlipperContainer) === null || _b === void 0 || _b.classList.toggle("disabled", this.validateStops(false) && Math.abs(position) + this.width >= lastStop);
		}
	}
	/**
	...
	*/
	scrollInView(item, padding = 0, rightPadding) {
		var _a;
		if (typeof item !== "number" && item) item = this.scrollItems.findIndex((scrollItem) => scrollItem === item || scrollItem.contains(item));
		if (item !== void 0) {
			...
			if (isBefore || scrollLeft + containerWidth - rightPadding < itemEnd) {
				const scrollTo = (_a = [...scrollStops].sort((a, b) => isBefore ? b - a : a - b).find((position) => isBefore ? position + padding < itemStart : position + containerWidth - (rightPadding !== null && rightPadding !== void 0 ? rightPadding : 0) > itemEnd)) !== null && _a !== void 0 ? _a : 0;
	...
	*/
	scrollToPrevious() {
		...
		if (nextIndex >= current || nextIndex === -1) nextIndex = current > 0 ? current - 1 : 0;
	...
	*/
	scrollToNext() {
		...
		if (outOfView > current + 2) nextIndex = outOfView - 2;
		else if (current < this.scrollStops.length - 2) nextIndex = current + 1;
		this.scrollToPosition(this.scrollStops[nextIndex], scrollPosition);
	}
	/**
	...
	*/
	scrollToPosition(newPosition, position = this.scrollContainer.scrollLeft) {
		...
		if (this.isRtl) transitionStop = this.scrollContainer.scrollLeft + Math.min(Math.abs(newPosition), maxScrollValue);
	...
	*/
	scrolled() {
		if (this.scrollTimeout) this.scrollTimeout = clearTimeout(this.scrollTimeout);
		this.scrollTimeout = setTimeout(() => {
			this.setFlippers();
		}, this.frameTime);
	}
};
__decorate$1([attr({ converter: nullableNumberConverter })], HorizontalScroll$1.prototype, "speed", void 0);
...
__decorate$1([attr({ attribute: "view" })], HorizontalScroll$1.prototype, "view", void 0);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/horizontal-scroll/horizontal-scroll.template.js
/**
* @public
*/
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/utilities/whitespace-filter.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/search/search.form-associated.js
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/search/search.js
/**
...
* @public
*/
var Search$1 = class extends FormAssociatedSearch {
	readOnlyChanged() {
		if (this.proxy instanceof HTMLInputElement) {
			this.proxy.readOnly = this.readOnly;
			this.validate();
...
__decorate$1([attr({ mode: "boolean" })], Search$1.prototype, "autofocus", void 0);
__decorate$1([attr], Search$1.prototype, "placeholder", void 0);
__decorate$1([attr], Search$1.prototype, "list", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], Search$1.prototype, "maxlength", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], Search$1.prototype, "minlength", void 0);
__decorate$1([attr], Search$1.prototype, "pattern", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], Search$1.prototype, "size", void 0);
__decorate$1([attr({ mode: "boolean" })], Search$1.prototype, "spellcheck", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/select/select.form-associated.js
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/select/select.js
/**
...
* @public
*/
var Select$1 = class extends FormAssociatedSelect {
	constructor() {
		super(...arguments);
		/**
		* The open attribute.
	...
	*/
	updateValue(shouldEmit) {
		var _a, _b;
		if (this.$fastController.isConnected) this.value = (_b = (_a = this.firstSelectedOption) === null || _a === void 0 ? void 0 : _a.value) !== null && _b !== void 0 ? _b : "";
		if (shouldEmit) {
			this.$emit("input");
	...
	*/
	clickHandler(e) {
		...
		if (!this.open && this.indexWhenOpened !== this.selectedIndex) this.updateValue(true);
		return true;
	}
	/**
	...
	*/
	mousedownHandler(e) {
		var _a;
		if (e.offsetX >= 0 && e.offsetX <= ((_a = this.listbox) === null || _a === void 0 ? void 0 : _a.scrollWidth)) return super.mousedownHandler(e);
	...
	*/
	selectedOptionsChanged(prev, next) {
		...
		(_a = this.options) === null || _a === void 0 || _a.forEach((o, i) => {
			var _a;
	...
	*/
	sizeChanged(prev, next) {
		...
		if (this.proxy) this.proxy.size = next;
	}
	/**
	*
	* @internal
...
__decorate$1([attr({ attribute: "position" })], Select$1.prototype, "positionAttribute", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/select/select.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/skeleton/skeleton.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/skeleton/skeleton.js
/**
...
__decorate$1([attr({ mode: "boolean" })], Skeleton.prototype, "shimmer", void 0);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/slider-label/slider-label.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/slider/slider-utilities.js
/**
* Converts a pixel coordinate on the track to a percent of the track's range
*/
function convertPixelToPercent(pixelPos, minPosition, maxPosition, direction) {
	let pct = limit(0, 1, (pixelPos - minPosition) / (maxPosition - minPosition));
	if (direction === Direction.rtl) pct = 1 - pct;
	return pct;
}
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/slider-label/slider-label.js
var defaultConfig = {
	min: 0,
	max: 0,
		...
		*/
		...
		this.getSliderConfiguration = () => {
			if (!this.isSliderConfig(this.parentNode)) {
				...
				const { min, max, direction, orientation, disabled } = this.parentNode;
				if (disabled !== void 0) this.disabled = disabled;
		...
		this.positionAsStyle = () => {
			...
			if (this.sliderOrientation === Orientation.horizontal) return direction === Direction.rtl ? `right: ${leftNum}%; left: ${rightNum}%;` : `left: ${leftNum}%; right: ${rightNum}%;`;
			else return `top: ${leftNum}%; bottom: ${rightNum}%;`;
		};
	}
	positionChanged() {
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/slider/slider.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/slider/slider.form-associated.js
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/slider/slider.js
/**
* The selection modes of a {@link @microsoft/fast-foundation#(Slider:class)}.
* @public
*/
var SliderMode = { singleValue: "single-value" };
/**
...
var Slider = class extends FormAssociatedSlider {
		...
		*/
		...
		this.keypressHandler = (e) => {
			...
			if (e.key === "Home") {
				...
				e.preventDefault();
				this.value = `${this.max}`;
			} else if (!e.shiftKey) switch (e.key) {
				case keyArrowRight:
				case keyArrowUp:
					e.preventDefault();
					this.increment();
		...
		this.setupListeners = (remove = false) => {
			...
			this.thumb[eventAction]("mousedown", this.handleThumbMouseDown, { passive: true });
			this.thumb[eventAction]("touchstart", this.handleThumbMouseDown, { passive: true });
			if (remove) {
	...
	*/
	valueChanged(previous, next) {
		...
		if (this.$fastController.isConnected) this.setThumbPositionForOrientation(this.direction);
		this.$emit("change");
	}
	minChanged() {
		if (this.proxy instanceof HTMLInputElement) this.proxy.min = `${this.min}`;
		this.validate();
	}
	maxChanged() {
		if (this.proxy instanceof HTMLInputElement) this.proxy.max = `${this.max}`;
		this.validate();
	}
	stepChanged() {
		if (this.proxy instanceof HTMLInputElement) this.proxy.step = `${this.step}`;
	...
	*/
	setThumbPositionForOrientation(direction) {
		const percentage = (1 - convertPixelToPercent(Number(this.value), Number(this.min), Number(this.max), direction)) * 100;
		if (this.orientation === Orientation.horizontal) this.position = this.isDragging ? `right: ${percentage}%; transition: none;` : `right: ${percentage}%; transition: all 0.2s ease;`;
		else this.position = this.isDragging ? `bottom: ${percentage}%; transition: none;` : `bottom: ${percentage}%; transition: all 0.2s ease;`;
	}
	/**
	...
	*/
	...
	setupDefaultValue() {
		if (typeof this.value === "string") if (this.value.length === 0) this.initialValue = this.midpoint;
			...
			const value = parseFloat(this.value);
			if (!Number.isNaN(value) && (value < this.min || value > this.max)) this.value = this.midpoint;
		}
	}
};
__decorate$1([attr({
...
__decorate$1([attr({ converter: nullableNumberConverter })], Slider.prototype, "min", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], Slider.prototype, "max", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], Slider.prototype, "step", void 0);
__decorate$1([attr], Slider.prototype, "orientation", void 0);
__decorate$1([attr], Slider.prototype, "mode", void 0);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/switch/switch.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/switch/switch.form-associated.js
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/switch/switch.js
/**
* A Switch Custom HTML Element.
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/tab-panel/tab-panel.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/tab-panel/tab-panel.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/tab/tab.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/tab/tab.js
/**
...
__decorate$1([attr({ mode: "boolean" })], Tab.prototype, "disabled", void 0);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/tabs/tabs.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/tabs/tabs.js
/**
...
var Tabs = class extends FoundationElement {
	constructor() {
		...
		*/
		...
		this.setTabs = () => {
			...
			this.tabs.forEach((tab, index) => {
				if (tab.slot === "tab") {
					...
					if (this.activeindicator && this.isFocusableElement(tab)) this.showActiveIndicator = true;
		...
		this.handleTabKeyDown = (event) => {
			if (this.isHorizontal()) switch (event.key) {
				case keyArrowLeft:
					event.preventDefault();
		...
		this.adjustForward = (e) => {
			...
			while (index < group.length && group.length > 1) if (this.isFocusableElement(group[index])) {
				this.moveToTabByIndex(group, index);
				break;
			} else if (this.activetab && index === group.indexOf(this.activetab)) break;
		...
		this.adjustBackward = (e) => {
			...
			index = index < 0 ? group.length - 1 : index;
			while (index >= 0 && group.length > 1) if (this.isFocusableElement(group[index])) {
				this.moveToTabByIndex(group, index);
				break;
			} else if (index - 1 < 0) index = group.length - 1;
			else index -= 1;
	...
	*/
	...
	getActiveIndex() {
		if (this.activeid !== void 0) return this.tabIds.indexOf(this.activeid) === -1 ? 0 : this.tabIds.indexOf(this.activeid);
		else return 0;
	}
	getTabIds() {
	...
	handleActiveIndicatorPosition() {
		if (this.showActiveIndicator && this.activeindicator && this.activeTabIndex !== this.prevActiveTabIndex) if (this.ticking) this.ticking = false;
		else {
			this.ticking = true;
	...
	*/
	adjust(adjustment) {
		...
		if (nextIndex > -1) this.moveToTabByIndex(this.tabs, nextIndex);
	}
	focusTab() {
...
__decorate$1([attr({ mode: "boolean" })], Tabs.prototype, "activeindicator", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/text-area/text-area.form-associated.js
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/text-area/text-area.options.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/text-area/text-area.js
/**
...
* @public
*/
var TextArea$1 = class extends FormAssociatedTextArea {
	constructor() {
		super(...arguments);
		/**
...
__decorate$1([attr({ mode: "boolean" })], TextArea$1.prototype, "readOnly", void 0);
__decorate$1([attr], TextArea$1.prototype, "resize", void 0);
__decorate$1([attr({ mode: "boolean" })], TextArea$1.prototype, "autofocus", void 0);
__decorate$1([attr({ attribute: "form" })], TextArea$1.prototype, "formId", void 0);
__decorate$1([attr], TextArea$1.prototype, "list", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], TextArea$1.prototype, "maxlength", void 0);
__decorate$1([attr({ converter: nullableNumberConverter })], TextArea$1.prototype, "minlength", void 0);
...
__decorate$1([attr({ mode: "boolean" })], TextArea$1.prototype, "spellcheck", void 0);
__decorate$1([observable], TextArea$1.prototype, "defaultSlottedNodes", void 0);
applyMixins(TextArea$1, DelegatesARIATextbox);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/text-area/text-area.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/text-field/text-field.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/toolbar/toolbar.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/utilities/root-active-element.js
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/toolbar/toolbar.js
/**
...
var ToolbarArrowKeyMap = Object.freeze({
	[ArrowKeys.ArrowUp]: { [Orientation.vertical]: -1 },
	[ArrowKeys.ArrowDown]: { [Orientation.vertical]: 1 },
	[ArrowKeys.ArrowLeft]: { [Orientation.horizontal]: {
		[Direction.ltr]: -1,
		[Direction.rtl]: 1
	} },
	[ArrowKeys.ArrowRight]: { [Orientation.horizontal]: {
		[Direction.ltr]: 1,
		[Direction.rtl]: -1
	} }
});
...
* @public
*/
var Toolbar$1 = class Toolbar$1 extends FoundationElement {
	constructor() {
		super(...arguments);
		/**
	...
	*/
	...
	slottedItemsChanged() {
		if (this.$fastController.isConnected) this.reduceFocusableElements();
	}
	/**
	...
	*/
	mouseDownHandler(e) {
		...
		if (activeIndex > -1 && this.activeIndex !== activeIndex) this.setFocusedElement(activeIndex);
		return true;
	}
	...
	*/
	keydownHandler(e) {
		const key = e.key;
		if (!(key in ArrowKeys) || e.defaultPrevented || e.shiftKey) return true;
		const incrementer = this.getDirectionalIncrementer(key);
		if (!incrementer) return !e.target.closest("[role=radiogroup]");
		const nextIndex = this.activeIndex + incrementer;
		if (this.focusableElements[nextIndex]) e.preventDefault();
	...
	*/
	setFocusedElement(activeIndex = this.activeIndex) {
		this.activeIndex = activeIndex;
		this.setFocusableElements();
		if (this.focusableElements[this.activeIndex] && this.contains(getRootActiveElement(this))) this.focusableElements[this.activeIndex].focus();
	}
	/**
	* Reduce a collection to only its focusable elements.
	*
	...
	*/
	static reduceFocusableItems(elements, element) {
		...
		if (element.childElementCount) return elements.concat(Array.from(element.children).reduce(Toolbar$1.reduceFocusableItems, []));
		return elements;
	}
	/**
	* @internal
	*/
	setFocusableElements() {
		if (this.$fastController.isConnected && this.focusableElements.length > 0) this.focusableElements.forEach((element, index) => {
...
__decorate$1([attr({ attribute: "aria-labelledby" })], DelegatesARIAToolbar.prototype, "ariaLabelledby", void 0);
__decorate$1([attr({ attribute: "aria-label" })], DelegatesARIAToolbar.prototype, "ariaLabel", void 0);
applyMixins(DelegatesARIAToolbar, ARIAGlobalStatesAndProperties);
applyMixins(Toolbar$1, StartEnd, DelegatesARIAToolbar);
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/tooltip/tooltip.template.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/tooltip/tooltip.options.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/tooltip/tooltip.js
/**
...
* @public
*/
var Tooltip$1 = class extends FoundationElement {
	constructor() {
		super(...arguments);
		/**
		...
		*/
		this.startShowDelayTimer = () => {
			if (this.isAnchorHoveredFocused) return;
			if (this.delay > 1) {
		...
		*/
		this.getAnchor = () => {
			...
			if (rootNode instanceof ShadowRoot) return rootNode.getElementById(this.anchor);
			return document.getElementById(this.anchor);
		};
		/**
		...
		*/
		this.handleDocumentKeydown = (e) => {
			if (!e.defaultPrevented && this.tooltipVisible) switch (e.key) {
				case keyEscape:
					this.isAnchorHoveredFocused = false;
					this.updateTooltipVisibility();
					this.$emit("dismiss");
		...
		*/
		this.updateTooltipVisibility = () => {
			if (this.visible === false) this.hideTooltip();
		...
		*/
		this.setRegionProps = () => {
			...
			this.region.addEventListener("mouseover", this.handleRegionMouseOver, { passive: true });
			this.region.addEventListener("mouseout", this.handleRegionMouseOut, { passive: true });
		};
	}
	visibleChanged() {
		if (this.$fastController.isConnected) {
	...
	anchorChanged() {
		if (this.$fastController.isConnected) this.anchorElement = this.getAnchor();
	}
	positionChanged() {
		if (this.$fastController.isConnected) this.updateLayout();
	}
	anchorElementChanged(oldValue) {
		if (this.$fastController.isConnected) {
			...
			if (this.anchorElement !== null && this.anchorElement !== void 0) {
				...
				this.anchorElement.addEventListener("focusin", this.handleAnchorFocusIn, { passive: true });
				this.anchorElement.addEventListener("focusout", this.handleAnchorFocusOut, { passive: true });
				const anchorId = this.anchorElement.id;
				if (this.anchorElement.parentElement !== null) this.anchorElement.parentElement.querySelectorAll(":hover").forEach((element) => {
					if (element.id === anchorId) this.startShowDelayTimer();
				});
			}
			if (this.region !== null && this.region !== void 0 && this.tooltipVisible) this.region.anchorElement = this.anchorElement;
			this.updateLayout();
		}
	}
	viewportElementChanged() {
		if (this.region !== null && this.region !== void 0) this.region.viewportElement = this.viewportElement;
		this.updateLayout();
	}
	connectedCallback() {
		super.connectedCallback();
...
__decorate$1([attr({ mode: "boolean" })], Tooltip$1.prototype, "visible", void 0);
...
__decorate$1([attr({ attribute: "auto-update-mode" })], Tooltip$1.prototype, "autoUpdateMode", void 0);
__decorate$1([attr({ attribute: "horizontal-viewport-lock" })], Tooltip$1.prototype, "horizontalViewportLock", void 0);
__decorate$1([attr({ attribute: "vertical-viewport-lock" })], Tooltip$1.prototype, "verticalViewportLock", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/tree-item/tree-item.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/tree-item/tree-item.js
/**
...
__decorate$1([attr({ mode: "boolean" })], TreeItem.prototype, "expanded", void 0);
__decorate$1([attr({ mode: "boolean" })], TreeItem.prototype, "selected", void 0);
__decorate$1([attr({ mode: "boolean" })], TreeItem.prototype, "disabled", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/tree-view/tree-view.template.js
/**
...
`;
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/tree-view/tree-view.js
/**
...
var TreeView = class extends FoundationElement {
	constructor() {
		...
		*/
		this.handleFocus = (e) => {
			if (this.slottedTreeItems.length < 1) return;
			if (e.target === this) {
				if (this.currentFocused === null) this.currentFocused = this.getValidFocusableItem();
...
__decorate$1([attr({ attribute: "render-collapsed-nodes" })], TreeView.prototype, "renderCollapsedNodes", void 0);
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/utilities/match-media-stylesheet-behavior.js
/**
...
* @public
*/
var MatchMediaStyleSheetBehavior = class MatchMediaStyleSheetBehavior extends MatchMediaBehavior {
	/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/utilities/property-stylesheet-behavior.js
/**
...
var PropertyStyleSheetBehavior = class {
	...
	*/
	handleChange(source, key) {
		if (source[key] === this.value) source.$fastController.addStyles(this.styles);
		else source.$fastController.removeStyles(this.styles);
	}
};
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/utilities/style/disabled.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/utilities/style/display.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-foundation/dist/esm/utilities/style/focus.js
/**
...
//#endregion
//#region node_modules/@microsoft/fast-colors/dist/math-utilities.js
/**
...
function lerp(i, min, max) {
	if (isNaN(i) || i <= 0) return min;
	else if (i >= 1) return max;
	return min + i * (max - min);
}
Math.PI * 2;
/**
...
//#endregion
//#region node_modules/@microsoft/fast-colors/dist/color-hsl.js
/**
...
var ColorHSL = class ColorHSL {
	...
	*/
	static fromObject(data) {
		if (data && !isNaN(data.h) && !isNaN(data.s) && !isNaN(data.l)) return new ColorHSL(data.h, data.s, data.l);
		return null;
	}
	/**
...
//#endregion
//#region node_modules/@microsoft/fast-colors/dist/color-lab.js
/**
...
var ColorLAB = class ColorLAB {
	...
	*/
	static fromObject(data) {
		if (data && !isNaN(data.l) && !isNaN(data.a) && !isNaN(data.b)) return new ColorLAB(data.l, data.a, data.b);
		return null;
	}
	/**
	* Determines if a color is equal to another
...
//#endregion
//#region node_modules/@microsoft/fast-colors/dist/color-rgba-64.js
/**
...
* @public
*/
var ColorRGBA64 = class ColorRGBA64 {
	/**
	*
...
//#endregion
//#region node_modules/@microsoft/fast-colors/dist/color-xyz.js
/**
...
var ColorXYZ = class ColorXYZ {
	...
	*/
	static fromObject(data) {
		if (data && !isNaN(data.x) && !isNaN(data.y) && !isNaN(data.z)) return new ColorXYZ(data.x, data.y, data.z);
		return null;
	}
	/**
	* Determines if a color is equal to another
...
*/
ColorXYZ.whitePoint = new ColorXYZ(.95047, 1, 1.08883);
//#endregion
//#region node_modules/@microsoft/fast-colors/dist/color-converters.js
/**
...
function rgbToRelativeLuminance(rgb) {
	function luminanceHelper(i) {
		if (i <= .03928) return i / 12.92;
		return Math.pow((i + .055) / 1.055, 2.4);
	}
...
function calcChannelOverlay(match, background, overlay) {
	if (overlay - background === 0) return 0;
	else return (match - background) / (overlay - background);
}
...
function calculateOverlayColor(rgbMatch, rgbBackground, rgbOverlay = null) {
	...
	if (overlay !== null) alpha = calcRgbOverlay(rgbMatch, rgbBackground, overlay);
	else {
...
function rgbToHSL(rgb) {
	...
	const delta = max - min;
	let hue = 0;
	if (delta !== 0) if (max === rgb.r) hue = 60 * ((rgb.g - rgb.b) / delta % 6);
	else if (max === rgb.g) hue = 60 * ((rgb.b - rgb.r) / delta + 2);
	else hue = 60 * ((rgb.r - rgb.g) / delta + 4);
	if (hue < 0) hue += 360;
	const lum = (max + min) / 2;
	let sat = 0;
	if (delta !== 0) sat = delta / (1 - Math.abs(2 * lum - 1));
...
function labToXYZ(lab) {
	...
	let x = 0;
	if (xcubed > ColorLAB.epsilon) x = xcubed;
	else x = (116 * fx - 16) / ColorLAB.kappa;
	let y = 0;
	if (lab.l > ColorLAB.epsilon * ColorLAB.kappa) y = ycubed;
	else y = lab.l / ColorLAB.kappa;
	let z = 0;
	if (zcubed > ColorLAB.epsilon) z = zcubed;
	else z = (116 * fz - 16) / ColorLAB.kappa;
...
function xyzToLAB(xyz) {
	function xyzToLABHelper(i) {
		if (i > ColorLAB.epsilon) return Math.pow(i, 1 / 3);
...
function rgbToXYZ(rgb) {
	function rgbToXYZHelper(i) {
		if (i <= .04045) return i / 12.92;
		return Math.pow((i + .055) / 1.055, 2.4);
	}
	const r = rgbToXYZHelper(rgb.r);
	const g = rgbToXYZHelper(rgb.g);
	const b = rgbToXYZHelper(rgb.b);
	return new ColorXYZ(r * .4124564 + g * .3575761 + b * .1804375, r * .2126729 + g * .7151522 + b * .072175, r * .0193339 + g * .119192 + b * .9503041);
}
/**
...
function xyzToRGB(xyz, alpha = 1) {
	function xyzToRGBHelper(i) {
		if (i <= .0031308) return i * 12.92;
		return 1.055 * Math.pow(i, 1 / 2.4) - .055;
	}
	return new ColorRGBA64(xyzToRGBHelper(xyz.x * 3.2404542 - xyz.y * 1.5371385 - xyz.z * .4985314), xyzToRGBHelper(xyz.x * -.969266 + xyz.y * 1.8760108 + xyz.z * .041556), xyzToRGBHelper(xyz.x * .0556434 - xyz.y * .2040259 + xyz.z * 1.0572252), alpha);
}
/**
...
//#endregion
//#region node_modules/@microsoft/fast-colors/dist/color-blending.js
/**
* Color blend modes.
* @public
*/
...
function computeAlphaBlend(bottom, top) {
	...
	else if (top.a <= 0) return new ColorRGBA64(bottom.r, bottom.g, bottom.b, 1);
	return new ColorRGBA64(top.a * top.r + (1 - top.a) * bottom.r, top.a * top.g + (1 - top.a) * bottom.g, top.a * top.b + (1 - top.a) * bottom.b, 1);
}
//#endregion
//#region node_modules/@microsoft/fast-colors/dist/color-interpolation.js
/**
...
function interpolateRGB(position, left, right) {
	...
	else if (position >= 1) return right;
...
//#endregion
//#region node_modules/@microsoft/fast-colors/dist/parse-color.js
...
function parseColorHexRGB(raw) {
	...
	if (isNaN(rawInt)) return null;
	return new ColorRGBA64(normalize((rawInt & 16711680) >>> 16, 0, 255), normalize((rawInt & 65280) >>> 8, 0, 255), normalize(rawInt & 255, 0, 255), 1);
}
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/utilities/relative-luminance.js
/**
* @internal
*/
function contrast(a, b) {
	...
	return (L1.relativeLuminance + .05) / (L2.relativeLuminance + .05);
}
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/swatch.js
/** @public */
...
function isSwatchRGB(value) {
	...
	for (const key in test) if (typeof test[key] !== typeof value[key]) return false;
	return true;
}
/**
...
* @internal
*/
var SwatchRGBImpl = class SwatchRGBImpl extends ColorRGBA64 {
	/**
	*
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/utilities/binary-search.js
/**
* @internal
*/
function binarySearch(valuesToSearch, searchCondition, startIndex = 0, endIndex = valuesToSearch.length - 1) {
	if (endIndex === startIndex) return valuesToSearch[startIndex];
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/utilities/is-dark.js
var target = (-.1 + Math.sqrt(.21)) / 2;
/**
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/utilities/direction-by-is-dark.js
/**
* @internal
*/
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/palette.js
...
function create$1(rOrSource, g, b) {
	if (typeof rOrSource === "number") return PaletteRGB.from(SwatchRGB.create(rOrSource, g, b));
	else return PaletteRGB.from(rOrSource);
}
...
* @public
*/
var PaletteRGBImpl = class PaletteRGBImpl {
	/**
	*
	...
	*/
	colorContrast(reference, contrastTarget, initialSearchIndex, direction) {
		if (initialSearchIndex === void 0) initialSearchIndex = this.closestIndexOf(reference);
		let source = this.swatches;
		const endSearchIndex = this.lastIndex;
		let startSearchIndex = initialSearchIndex;
		if (direction === void 0) direction = directionByIsDark(reference);
	...
	*/
	closestIndexOf(reference) {
		if (this.closestIndexCache.has(reference.relativeLuminance)) return this.closestIndexCache.get(reference.relativeLuminance);
	...
	*/
	static saturationBump(reference, color) {
		const saturationTarget = rgbToHSL(reference).s;
		const hslColor = rgbToHSL(color);
		if (hslColor.s < saturationTarget) return hslToRGB(new ColorHSL(hslColor.h, saturationTarget, hslColor.l));
		return color;
	}
	/**
	...
	*/
	static ramp(l) {
		...
		if (inputval > .5) return (inputval - .5) / .5;
		return 2 * inputval;
	}
	/**
	...
	*/
	static createHighResolutionPalette(source) {
		...
		for (let l = 100 + lAbove; l >= 0 - lBelow; l -= .5) {
			let rgb;
			if (l < 0) rgb = interpolateRGB(l / lBelow + 1, rgbMin, lab0);
			else if (l <= 50) rgb = interpolateRGB(PaletteRGBImpl.ramp(l), lab0, lab50);
			else if (l <= 100) rgb = interpolateRGB(PaletteRGBImpl.ramp(l), lab50, lab100);
			else rgb = interpolateRGB((l - 100) / lAbove, lab100, rgbMax);
	...
	*/
	static adjustEnd(swatchContrast, referencePalette, targetPalette, direction) {
		...
		if (direction === 1) targetPalette.reverse();
		const targetContrast = swatchContrast(targetPalette[targetPalette.length - 2]);
		if (roundToPrecisionSmall(contrast(targetPalette[targetPalette.length - 1], targetPalette[targetPalette.length - 2]), 2) < targetContrast) {
			targetPalette.pop();
			const swatchesToSpace = refIndex(referencePalette.colorContrast(refSwatches[referencePalette.lastIndex], targetContrast, void 0, direction)) - refIndex(targetPalette[targetPalette.length - 2]);
			let space = 1;
			for (let i = targetPalette.length - swatchesToSpace - 1; i < targetPalette.length; i++) {
				const currentRefIndex = refIndex(targetPalette[i]);
				targetPalette[i] = refSwatches[i === targetPalette.length - 1 ? referencePalette.lastIndex : currentRefIndex + space];
				space++;
			}
		}
		if (direction === 1) targetPalette.reverse();
	...
	*/
	static createColorPaletteByContrast(source, options) {
		...
		const nextContrast = (swatch) => {
			return roundToPrecisionSmall(options.stepContrast + options.stepContrast * (1 - swatch.relativeLuminance) * options.stepContrastRamp, 2);
		};
		const swatches = [];
		...
		do {
			...
			ref = referencePalette.colorContrast(ref, targetContrast, void 0, 1);
			swatches.push(ref);
		...
		if (options.preserveSource) this.adjustEnd(nextContrast, referencePalette, swatches, 1);
		return swatches;
	}
	/**
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/utilities/color-constants.js
/**
* @internal
*/
var white$1 = SwatchRGB.create(1, 1, 1);
/**
* @internal
*/
var black$1 = SwatchRGB.create(0, 0, 0);
/**
* @internal
*/
var middleGrey = SwatchRGB.create(.5, .5, .5);
/**
* @internal
*/
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/foreground-on-accent.js
function foregroundOnAccentSet(restFill, hoverFill, activeFill, focusFill, contrastTarget) {
	...
	return {
		...
		active: restForeground.relativeLuminance === hoverForeground.relativeLuminance ? restForeground : defaultRule(activeFill),
		focus: defaultRule(focusFill)
	};
}
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/gradient-swatch.js
/**
...
* @internal
*/
var GradientSwatchRGB = class GradientSwatchRGB {
	/**
	*
	* @param red Red channel expressed as a number between 0 and 1
	* @param green Green channel expressed as a number between 0 and 1
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/gradient-shadow-stroke.js
var black = new ColorRGBA64(0, 0, 0);
var white = new ColorRGBA64(1, 1, 1);
/**
* @internal
*/
function gradientShadowStroke(palette, reference, restDelta, hoverDelta, activeDelta, focusDelta, shadowDelta, direction, shadowPercentage = 10, blendWithReference = false) {
	...
	function gradientHelper(index, applyShadow) {
		...
		} else return overlayHelper(color);
	}
	return {
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/underline-stroke.js
/**
* @internal
*/
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/contrast-swatch.js
/**
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/contrast-and-delta-swatch-set.js
/**
* @internal
*/
function contrastAndDeltaSwatchSet(palette, reference, baseContrast, restDelta, hoverDelta, activeDelta, focusDelta, direction) {
	if (direction === null || direction === void 0) direction = directionByIsDark(reference);
...
function contrastAndDeltaSwatchSetByLuminance(palette, reference, lightBaseContrast, lightRestDelta, lightHoverDelta, lightActiveDelta, lightFocusDelta, lightDirection = void 0, darkBaseContrast, darkRestDelta, darkHoverDelta, darkActiveDelta, darkFocusDelta, darkDirection = void 0) {
	if (isDark(reference)) return contrastAndDeltaSwatchSet(palette, reference, darkBaseContrast, darkRestDelta, darkHoverDelta, darkActiveDelta, darkFocusDelta, darkDirection);
	else return contrastAndDeltaSwatchSet(palette, reference, lightBaseContrast, lightRestDelta, lightHoverDelta, lightActiveDelta, lightFocusDelta, lightDirection);
}
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/delta-swatch.js
/**
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/delta-swatch-set.js
/**
...
function deltaSwatchSetByLuminance(palette, reference, lightRestDelta, lightHoverDelta, lightActiveDelta, lightFocusDelta, lightDirection = void 0, darkRestDelta, darkHoverDelta, darkActiveDelta, darkFocusDelta, darkDirection = void 0) {
	if (isDark(reference)) return deltaSwatchSet(palette, reference, darkRestDelta, darkHoverDelta, darkActiveDelta, darkFocusDelta, darkDirection);
	else return deltaSwatchSet(palette, reference, lightRestDelta, lightHoverDelta, lightActiveDelta, lightFocusDelta, lightDirection);
}
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/focus-stroke.js
/**
* @internal
*/
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/utilities/base-layer-luminance.js
...
(function(StandardLuminance) {
	StandardLuminance[StandardLuminance["LightMode"] = .98] = "LightMode";
	StandardLuminance[StandardLuminance["DarkMode"] = .15] = "DarkMode";
})(StandardLuminance || (StandardLuminance = {}));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/neutral-layer-1.js
/**
* @internal
*/
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/neutral-layer-floating.js
/**
* @internal
*/
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/neutral-layer-2.js
/**
* @internal
*/
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/neutral-layer-3.js
/**
* @internal
*/
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/color/recipes/neutral-layer-4.js
/**
* @internal
*/
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/utilities/type-ramp.js
/** @public */
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/design-tokens.js
var { create } = DesignToken;
...
/** @public */
var direction = create("direction").withDefault(Direction.ltr);
/** @public */
var disabledOpacity = create("disabled-opacity").withDefault(.3);
/** @public */
var baseHeightMultiplier = create("base-height-multiplier").withDefault(8);
/** @public */
var baseHorizontalSpacingMultiplier = create("base-horizontal-spacing-multiplier").withDefault(3);
/** @public */
var density = create("density").withDefault(0);
/** @public */
var designUnit = create("design-unit").withDefault(4);
/** @public */
var controlCornerRadius = create("control-corner-radius").withDefault(4);
/** @public */
var layerCornerRadius = create("layer-corner-radius").withDefault(8);
/** @public */
var strokeWidth = create("stroke-width").withDefault(1);
/** @public */
var focusStrokeWidth = create("focus-stroke-width").withDefault(2);
/** @public */
var bodyFont = create("body-font").withDefault("\"Segoe UI Variable\", \"Segoe UI\", sans-serif");
/** @public */
var fontWeight = create("font-weight").withDefault(StandardFontWeight.Normal);
function fontVariations(sizeToken) {
	return (element) => {
		...
		if (size.endsWith("px")) {
			...
			if (px <= 12) return `"wght" ${weight}, "opsz" 8`;
			else if (px > 24) return `"wght" ${weight}, "opsz" 36`;
		}
		return `"wght" ${weight}, "opsz" 10.5`;
	};
}
/** @public */
var typeRampBaseFontSize = create("type-ramp-base-font-size").withDefault("14px");
/** @public */
var typeRampBaseLineHeight = create("type-ramp-base-line-height").withDefault("20px");
/** @public */
var typeRampBaseFontVariations = create("type-ramp-base-font-variations").withDefault(fontVariations(typeRampBaseFontSize));
/** @public */
var typeRampMinus1FontSize = create("type-ramp-minus-1-font-size").withDefault("12px");
/** @public */
var typeRampMinus1LineHeight = create("type-ramp-minus-1-line-height").withDefault("16px");
/** @public */
var typeRampMinus1FontVariations = create("type-ramp-minus-1-font-variations").withDefault(fontVariations(typeRampMinus1FontSize));
/** @public */
var typeRampMinus2FontSize = create("type-ramp-minus-2-font-size").withDefault("10px");
/** @public */
var typeRampMinus2LineHeight = create("type-ramp-minus-2-line-height").withDefault("14px");
/** @public */
var typeRampMinus2FontVariations = create("type-ramp-minus-2-font-variations").withDefault(fontVariations(typeRampMinus2FontSize));
/** @public */
var typeRampPlus1FontSize = create("type-ramp-plus-1-font-size").withDefault("16px");
/** @public */
var typeRampPlus1LineHeight = create("type-ramp-plus-1-line-height").withDefault("22px");
/** @public */
var typeRampPlus1FontVariations = create("type-ramp-plus-1-font-variations").withDefault(fontVariations(typeRampPlus1FontSize));
/** @public */
var typeRampPlus2FontSize = create("type-ramp-plus-2-font-size").withDefault("20px");
/** @public */
var typeRampPlus2LineHeight = create("type-ramp-plus-2-line-height").withDefault("26px");
/** @public */
var typeRampPlus2FontVariations = create("type-ramp-plus-2-font-variations").withDefault(fontVariations(typeRampPlus2FontSize));
/** @public */
var typeRampPlus3FontSize = create("type-ramp-plus-3-font-size").withDefault("24px");
/** @public */
var typeRampPlus3LineHeight = create("type-ramp-plus-3-line-height").withDefault("32px");
/** @public */
var typeRampPlus3FontVariations = create("type-ramp-plus-3-font-variations").withDefault(fontVariations(typeRampPlus3FontSize));
/** @public */
var typeRampPlus4FontSize = create("type-ramp-plus-4-font-size").withDefault("28px");
/** @public */
var typeRampPlus4LineHeight = create("type-ramp-plus-4-line-height").withDefault("36px");
/** @public */
var typeRampPlus4FontVariations = create("type-ramp-plus-4-font-variations").withDefault(fontVariations(typeRampPlus4FontSize));
/** @public */
var typeRampPlus5FontSize = create("type-ramp-plus-5-font-size").withDefault("32px");
/** @public */
var typeRampPlus5LineHeight = create("type-ramp-plus-5-line-height").withDefault("40px");
/** @public */
var typeRampPlus5FontVariations = create("type-ramp-plus-5-font-variations").withDefault(fontVariations(typeRampPlus5FontSize));
/** @public */
var typeRampPlus6FontSize = create("type-ramp-plus-6-font-size").withDefault("40px");
/** @public */
var typeRampPlus6LineHeight = create("type-ramp-plus-6-line-height").withDefault("52px");
/** @public */
var typeRampPlus6FontVariations = create("type-ramp-plus-6-font-variations").withDefault(fontVariations(typeRampPlus6FontSize));
/** @public */
var baseLayerLuminance = create("base-layer-luminance").withDefault(StandardLuminance.LightMode);
/** @public */
...
/** @public */
var neutralBaseColor = create("neutral-base-color").withDefault(middleGrey);
/** @public */
var neutralPalette = createNonCss("neutral-palette").withDefault((element) => PaletteRGB.from(neutralBaseColor.getValueFor(element)));
/** @public */
var accentBaseColor = create("accent-base-color").withDefault(accentBase);
/** @public */
var accentPalette = createNonCss("accent-palette").withDefault((element) => PaletteRGB.from(accentBaseColor.getValueFor(element)));
/** @public */
var neutralLayerCardContainerRecipe = createNonCss("neutral-layer-card-container-recipe").withDefault({ evaluate: (element) => neutralLayer2$1(neutralPalette.getValueFor(element), baseLayerLuminance.getValueFor(element), neutralFillLayerRestDelta.getValueFor(element)) });
/** @public */
var neutralLayerCardContainer = create("neutral-layer-card-container").withDefault((element) => neutralLayerCardContainerRecipe.getValueFor(element).evaluate(element));
/** @public */
var neutralLayerFloatingRecipe = createNonCss("neutral-layer-floating-recipe").withDefault({ evaluate: (element) => neutralLayerFloating$1(neutralPalette.getValueFor(element), baseLayerLuminance.getValueFor(element), neutralFillLayerRestDelta.getValueFor(element)) });
/** @public */
var neutralLayerFloating = create("neutral-layer-floating").withDefault((element) => neutralLayerFloatingRecipe.getValueFor(element).evaluate(element));
/** @public */
var neutralLayer1Recipe = createNonCss("neutral-layer-1-recipe").withDefault({ evaluate: (element) => neutralLayer1$1(neutralPalette.getValueFor(element), baseLayerLuminance.getValueFor(element)) });
/** @public */
var neutralLayer1 = create("neutral-layer-1").withDefault((element) => neutralLayer1Recipe.getValueFor(element).evaluate(element));
/** @public */
var neutralLayer2Recipe = createNonCss("neutral-layer-2-recipe").withDefault({ evaluate: (element) => neutralLayer2$1(neutralPalette.getValueFor(element), baseLayerLuminance.getValueFor(element), neutralFillLayerRestDelta.getValueFor(element)) });
/** @public */
var neutralLayer2 = create("neutral-layer-2").withDefault((element) => neutralLayer2Recipe.getValueFor(element).evaluate(element));
/** @public */
var neutralLayer3Recipe = createNonCss("neutral-layer-3-recipe").withDefault({ evaluate: (element) => neutralLayer3$1(neutralPalette.getValueFor(element), baseLayerLuminance.getValueFor(element), neutralFillLayerRestDelta.getValueFor(element)) });
/** @public */
var neutralLayer3 = create("neutral-layer-3").withDefault((element) => neutralLayer3Recipe.getValueFor(element).evaluate(element));
/** @public */
var neutralLayer4Recipe = createNonCss("neutral-layer-4-recipe").withDefault({ evaluate: (element) => neutralLayer4$1(neutralPalette.getValueFor(element), baseLayerLuminance.getValueFor(element), neutralFillLayerRestDelta.getValueFor(element)) });
/** @public */
var neutralLayer4 = create("neutral-layer-4").withDefault((element) => neutralLayer4Recipe.getValueFor(element).evaluate(element));
/** @public */
var fillColor = create("fill-color").withDefault((element) => neutralLayer1.getValueFor(element));
var ContrastTarget;
...
/** @public */
var accentFillRecipe = createNonCss("accent-fill-recipe").withDefault({ evaluate: (element, reference) => contrastAndDeltaSwatchSetByLuminance(accentPalette.getValueFor(element), reference || fillColor.getValueFor(element), 5, accentFillRestDelta.getValueFor(element), accentFillHoverDelta.getValueFor(element), accentFillActiveDelta.getValueFor(element), accentFillFocusDelta.getValueFor(element), void 0, 8, accentFillRestDelta.getValueFor(element), accentFillHoverDelta.getValueFor(element), accentFillActiveDelta.getValueFor(element), accentFillFocusDelta.getValueFor(element), void 0) });
/** @public */
var accentFillRest = create("accent-fill-rest").withDefault((element) => {
	return accentFillRecipe.getValueFor(element).evaluate(element).rest;
});
/** @public */
var accentFillHover = create("accent-fill-hover").withDefault((element) => {
	return accentFillRecipe.getValueFor(element).evaluate(element).hover;
});
/** @public */
var accentFillActive = create("accent-fill-active").withDefault((element) => {
	return accentFillRecipe.getValueFor(element).evaluate(element).active;
});
/** @public */
var accentFillFocus = create("accent-fill-focus").withDefault((element) => {
	return accentFillRecipe.getValueFor(element).evaluate(element).focus;
});
/** @public */
var foregroundOnAccentRecipe = createNonCss("foreground-on-accent-recipe").withDefault({ evaluate: (element) => foregroundOnAccentSet(accentFillRest.getValueFor(element), accentFillHover.getValueFor(element), accentFillActive.getValueFor(element), accentFillFocus.getValueFor(element), ContrastTarget.normal) });
/** @public */
var foregroundOnAccentRest = create("foreground-on-accent-rest").withDefault((element) => foregroundOnAccentRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var foregroundOnAccentHover = create("foreground-on-accent-hover").withDefault((element) => foregroundOnAccentRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var foregroundOnAccentActive = create("foreground-on-accent-active").withDefault((element) => foregroundOnAccentRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var foregroundOnAccentFocus = create("foreground-on-accent-focus").withDefault((element) => foregroundOnAccentRecipe.getValueFor(element).evaluate(element).focus);
/** @public */
var accentForegroundRecipe = createNonCss("accent-foreground-recipe").withDefault({ evaluate: (element, reference) => contrastAndDeltaSwatchSet(accentPalette.getValueFor(element), reference || fillColor.getValueFor(element), 9.5, accentForegroundRestDelta.getValueFor(element), accentForegroundHoverDelta.getValueFor(element), accentForegroundActiveDelta.getValueFor(element), accentForegroundFocusDelta.getValueFor(element)) });
/** @public */
var accentForegroundRest = create("accent-foreground-rest").withDefault((element) => accentForegroundRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var accentForegroundHover = create("accent-foreground-hover").withDefault((element) => accentForegroundRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var accentForegroundActive = create("accent-foreground-active").withDefault((element) => accentForegroundRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var accentForegroundFocus = create("accent-foreground-focus").withDefault((element) => accentForegroundRecipe.getValueFor(element).evaluate(element).focus);
/** @public */
var accentStrokeControlRecipe = createNonCss("accent-stroke-control-recipe").withDefault({ evaluate: (element, reference) => {
...
/** @public */
var accentStrokeControlRest = create("accent-stroke-control-rest").withDefault((element) => accentStrokeControlRecipe.getValueFor(element).evaluate(element, accentFillRest.getValueFor(element)).rest);
/** @public */
var accentStrokeControlHover = create("accent-stroke-control-hover").withDefault((element) => accentStrokeControlRecipe.getValueFor(element).evaluate(element, accentFillHover.getValueFor(element)).hover);
/** @public */
var accentStrokeControlActive = create("accent-stroke-control-active").withDefault((element) => accentStrokeControlRecipe.getValueFor(element).evaluate(element, accentFillActive.getValueFor(element)).active);
/** @public */
var accentStrokeControlFocus = create("accent-stroke-control-focus").withDefault((element) => accentStrokeControlRecipe.getValueFor(element).evaluate(element, accentFillFocus.getValueFor(element)).focus);
/** @public */
var neutralFillRecipe = createNonCss("neutral-fill-recipe").withDefault({ evaluate: (element, reference) => deltaSwatchSetByLuminance(neutralPalette.getValueFor(element), reference || fillColor.getValueFor(element), neutralFillRestDelta.getValueFor(element), neutralFillHoverDelta.getValueFor(element), neutralFillActiveDelta.getValueFor(element), neutralFillFocusDelta.getValueFor(element), void 0, 2, 3, 1, 2, void 0) });
/** @public */
var neutralFillRest = create("neutral-fill-rest").withDefault((element) => neutralFillRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralFillHover = create("neutral-fill-hover").withDefault((element) => neutralFillRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var neutralFillActive = create("neutral-fill-active").withDefault((element) => neutralFillRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var neutralFillFocus = create("neutral-fill-focus").withDefault((element) => neutralFillRecipe.getValueFor(element).evaluate(element).focus);
/** @public */
var neutralFillInputRecipe = createNonCss("neutral-fill-input-recipe").withDefault({ evaluate: (element, reference) => deltaSwatchSetByLuminance(neutralPalette.getValueFor(element), reference || fillColor.getValueFor(element), neutralFillInputRestDelta.getValueFor(element), neutralFillInputHoverDelta.getValueFor(element), neutralFillInputActiveDelta.getValueFor(element), neutralFillInputFocusDelta.getValueFor(element), void 0, 2, 3, 1, 0, void 0) });
/** @public */
var neutralFillInputRest = create("neutral-fill-input-rest").withDefault((element) => neutralFillInputRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralFillInputHover = create("neutral-fill-input-hover").withDefault((element) => neutralFillInputRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var neutralFillInputActive = create("neutral-fill-input-active").withDefault((element) => neutralFillInputRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var neutralFillInputFocus = create("neutral-fill-input-focus").withDefault((element) => neutralFillInputRecipe.getValueFor(element).evaluate(element).focus);
/** @public */
var neutralFillInputAltRecipe = createNonCss("neutral-fill-input-alt-recipe").withDefault({ evaluate: (element, reference) => deltaSwatchSetByLuminance(neutralPalette.getValueFor(element), reference || fillColor.getValueFor(element), neutralFillInputAltRestDelta.getValueFor(element), neutralFillInputAltHoverDelta.getValueFor(element), neutralFillInputAltActiveDelta.getValueFor(element), neutralFillInputAltFocusDelta.getValueFor(element), 1, neutralFillInputAltRestDelta.getValueFor(element), neutralFillInputAltRestDelta.getValueFor(element) - neutralFillInputAltHoverDelta.getValueFor(element), neutralFillInputAltRestDelta.getValueFor(element) - neutralFillInputAltActiveDelta.getValueFor(element), neutralFillInputAltFocusDelta.getValueFor(element), 1) });
/** @public */
var neutralFillInputAltRest = create("neutral-fill-input-alt-rest").withDefault((element) => neutralFillInputAltRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralFillInputAltHover = create("neutral-fill-input-alt-hover").withDefault((element) => neutralFillInputAltRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var neutralFillInputAltActive = create("neutral-fill-input-alt-active").withDefault((element) => neutralFillInputAltRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var neutralFillInputAltFocus = create("neutral-fill-input-alt-focus").withDefault((element) => neutralFillInputAltRecipe.getValueFor(element).evaluate(element).focus);
/** @public */
var neutralFillLayerRecipe = createNonCss("neutral-fill-layer-recipe").withDefault({ evaluate: (element, reference) => deltaSwatchSet(neutralPalette.getValueFor(element), reference || fillColor.getValueFor(element), neutralFillLayerRestDelta.getValueFor(element), neutralFillLayerHoverDelta.getValueFor(element), neutralFillLayerActiveDelta.getValueFor(element), neutralFillLayerRestDelta.getValueFor(element), 1) });
/** @public */
var neutralFillLayerRest = create("neutral-fill-layer-rest").withDefault((element) => neutralFillLayerRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralFillLayerHover = create("neutral-fill-layer-hover").withDefault((element) => neutralFillLayerRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var neutralFillLayerActive = create("neutral-fill-layer-active").withDefault((element) => neutralFillLayerRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var neutralFillLayerAltRecipe = createNonCss("neutral-fill-layer-alt-recipe").withDefault({ evaluate: (element, reference) => deltaSwatchSet(neutralPalette.getValueFor(element), reference || fillColor.getValueFor(element), neutralFillLayerAltRestDelta.getValueFor(element), neutralFillLayerAltRestDelta.getValueFor(element), neutralFillLayerAltRestDelta.getValueFor(element), neutralFillLayerAltRestDelta.getValueFor(element)) });
/** @public */
var neutralFillLayerAltRest = create("neutral-fill-layer-alt-rest").withDefault((element) => neutralFillLayerAltRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralFillSecondaryRecipe = createNonCss("neutral-fill-secondary-recipe").withDefault({ evaluate: (element, reference) => deltaSwatchSet(neutralPalette.getValueFor(element), reference || fillColor.getValueFor(element), neutralFillSecondaryRestDelta.getValueFor(element), neutralFillSecondaryHoverDelta.getValueFor(element), neutralFillSecondaryActiveDelta.getValueFor(element), neutralFillSecondaryFocusDelta.getValueFor(element)) });
/** @public */
var neutralFillSecondaryRest = create("neutral-fill-secondary-rest").withDefault((element) => neutralFillSecondaryRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralFillSecondaryHover = create("neutral-fill-secondary-hover").withDefault((element) => neutralFillSecondaryRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var neutralFillSecondaryActive = create("neutral-fill-secondary-active").withDefault((element) => neutralFillSecondaryRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var neutralFillSecondaryFocus = create("neutral-fill-secondary-focus").withDefault((element) => neutralFillSecondaryRecipe.getValueFor(element).evaluate(element).focus);
/** @public */
var neutralFillStealthRecipe = createNonCss("neutral-fill-stealth-recipe").withDefault({ evaluate: (element, reference) => deltaSwatchSet(neutralPalette.getValueFor(element), reference || fillColor.getValueFor(element), neutralFillStealthRestDelta.getValueFor(element), neutralFillStealthHoverDelta.getValueFor(element), neutralFillStealthActiveDelta.getValueFor(element), neutralFillStealthFocusDelta.getValueFor(element)) });
/** @public */
var neutralFillStealthRest = create("neutral-fill-stealth-rest").withDefault((element) => neutralFillStealthRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralFillStealthHover = create("neutral-fill-stealth-hover").withDefault((element) => neutralFillStealthRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var neutralFillStealthActive = create("neutral-fill-stealth-active").withDefault((element) => neutralFillStealthRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var neutralFillStealthFocus = create("neutral-fill-stealth-focus").withDefault((element) => neutralFillStealthRecipe.getValueFor(element).evaluate(element).focus);
/** @public */
var neutralFillStrongRecipe = createNonCss("neutral-fill-strong-recipe").withDefault({ evaluate: (element, reference) => contrastAndDeltaSwatchSet(neutralPalette.getValueFor(element), reference || fillColor.getValueFor(element), 4.5, neutralFillStrongRestDelta.getValueFor(element), neutralFillStrongHoverDelta.getValueFor(element), neutralFillStrongActiveDelta.getValueFor(element), neutralFillStrongFocusDelta.getValueFor(element)) });
/** @public */
var neutralFillStrongRest = create("neutral-fill-strong-rest").withDefault((element) => neutralFillStrongRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralFillStrongHover = create("neutral-fill-strong-hover").withDefault((element) => neutralFillStrongRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var neutralFillStrongActive = create("neutral-fill-strong-active").withDefault((element) => neutralFillStrongRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var neutralFillStrongFocus = create("neutral-fill-strong-focus").withDefault((element) => neutralFillStrongRecipe.getValueFor(element).evaluate(element).focus);
/** @public */
var neutralForegroundRecipe = createNonCss("neutral-foreground-recipe").withDefault({ evaluate: (element, reference) => contrastAndDeltaSwatchSet(neutralPalette.getValueFor(element), reference || fillColor.getValueFor(element), 16, 0, -19, -30, 0) });
/** @public */
var neutralForegroundRest = create("neutral-foreground-rest").withDefault((element) => neutralForegroundRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralForegroundHover = create("neutral-foreground-hover").withDefault((element) => neutralForegroundRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var neutralForegroundActive = create("neutral-foreground-active").withDefault((element) => neutralForegroundRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var neutralForegroundFocus = create("neutral-foreground-focus").withDefault((element) => neutralForegroundRecipe.getValueFor(element).evaluate(element).focus);
/** @public */
var neutralForegroundHintRecipe = createNonCss("neutral-foreground-hint-recipe").withDefault({ evaluate: (element, reference) => contrastSwatch(neutralPalette.getValueFor(element), reference || fillColor.getValueFor(element), 4.5) });
/** @public */
var neutralForegroundHint = create("neutral-foreground-hint").withDefault((element) => neutralForegroundHintRecipe.getValueFor(element).evaluate(element));
/** @public */
var neutralStrokeRecipe = createNonCss("neutral-stroke-recipe").withDefault({ evaluate: (element, reference) => {
...
/** @public */
var neutralStrokeRest = create("neutral-stroke-rest").withDefault((element) => neutralStrokeRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralStrokeHover = create("neutral-stroke-hover").withDefault((element) => neutralStrokeRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var neutralStrokeActive = create("neutral-stroke-active").withDefault((element) => neutralStrokeRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var neutralStrokeFocus = create("neutral-stroke-focus").withDefault((element) => neutralStrokeRecipe.getValueFor(element).evaluate(element).focus);
/** @public */
var neutralStrokeControlRecipe = createNonCss("neutral-stroke-control-recipe").withDefault({ evaluate: (element, reference) => {
...
/** @public */
var neutralStrokeControlRest = create("neutral-stroke-control-rest").withDefault((element) => neutralStrokeControlRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralStrokeControlHover = create("neutral-stroke-control-hover").withDefault((element) => neutralStrokeControlRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var neutralStrokeControlActive = create("neutral-stroke-control-active").withDefault((element) => neutralStrokeControlRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var neutralStrokeControlFocus = create("neutral-stroke-control-focus").withDefault((element) => neutralStrokeControlRecipe.getValueFor(element).evaluate(element).focus);
/** @public */
var neutralStrokeDividerRecipe = createNonCss("neutral-stroke-divider-recipe").withDefault({ evaluate: (element, reference) => deltaSwatch(neutralPalette.getValueFor(element), reference || fillColor.getValueFor(element), neutralStrokeDividerRestDelta.getValueFor(element)) });
/** @public */
var neutralStrokeDividerRest = create("neutral-stroke-divider-rest").withDefault((element) => neutralStrokeDividerRecipe.getValueFor(element).evaluate(element));
/** @public */
var neutralStrokeInputRecipe = createNonCss("neutral-stroke-input-recipe").withDefault({ evaluate: (element, reference) => {
...
/** @public */
var neutralStrokeInputRest = create("neutral-stroke-input-rest").withDefault((element) => neutralStrokeInputRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralStrokeInputHover = create("neutral-stroke-input-hover").withDefault((element) => neutralStrokeInputRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var neutralStrokeInputActive = create("neutral-stroke-input-active").withDefault((element) => neutralStrokeInputRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var neutralStrokeInputFocus = create("neutral-stroke-input-focus").withDefault((element) => neutralStrokeInputRecipe.getValueFor(element).evaluate(element).focus);
/** @public */
var neutralStrokeLayerRecipe = createNonCss("neutral-stroke-layer-recipe").withDefault({ evaluate: (element, reference) => {
...
/** @public */
var neutralStrokeLayerRest = create("neutral-stroke-layer-rest").withDefault((element) => neutralStrokeLayerRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralStrokeLayerHover = create("neutral-stroke-layer-hover").withDefault((element) => neutralStrokeLayerRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var neutralStrokeLayerActive = create("neutral-stroke-layer-active").withDefault((element) => neutralStrokeLayerRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var neutralStrokeStrongRecipe = createNonCss("neutral-stroke-strong-recipe").withDefault({ evaluate: (element, reference) => contrastAndDeltaSwatchSet(neutralPalette.getValueFor(element), reference || fillColor.getValueFor(element), 5.5, 0, neutralStrokeStrongHoverDelta.getValueFor(element), neutralStrokeStrongActiveDelta.getValueFor(element), neutralStrokeStrongFocusDelta.getValueFor(element)) });
/** @public */
var neutralStrokeStrongRest = create("neutral-stroke-strong-rest").withDefault((element) => neutralStrokeStrongRecipe.getValueFor(element).evaluate(element).rest);
/** @public */
var neutralStrokeStrongHover = create("neutral-stroke-strong-hover").withDefault((element) => neutralStrokeStrongRecipe.getValueFor(element).evaluate(element).hover);
/** @public */
var neutralStrokeStrongActive = create("neutral-stroke-strong-active").withDefault((element) => neutralStrokeStrongRecipe.getValueFor(element).evaluate(element).active);
/** @public */
var neutralStrokeStrongFocus = create("neutral-stroke-strong-focus").withDefault((element) => neutralStrokeStrongRecipe.getValueFor(element).evaluate(element).focus);
/** @public */
var focusStrokeOuterRecipe = createNonCss("focus-stroke-outer-recipe").withDefault({ evaluate: (element) => focusStrokeOuter$1(neutralPalette.getValueFor(element), fillColor.getValueFor(element)) });
/** @public */
var focusStrokeOuter = create("focus-stroke-outer").withDefault((element) => focusStrokeOuterRecipe.getValueFor(element).evaluate(element));
/** @public */
var focusStrokeInnerRecipe = createNonCss("focus-stroke-inner-recipe").withDefault({ evaluate: (element) => focusStrokeInner$1(accentPalette.getValueFor(element), fillColor.getValueFor(element), focusStrokeOuter.getValueFor(element)) });
/** @public */
var focusStrokeInner = create("focus-stroke-inner").withDefault((element) => focusStrokeInnerRecipe.getValueFor(element).evaluate(element));
/** @public @deprecated Not used */
var foregroundOnAccentLargeRecipe = createNonCss("foreground-on-accent-large-recipe").withDefault({ evaluate: (element) => foregroundOnAccentSet(accentFillRest.getValueFor(element), accentFillHover.getValueFor(element), accentFillActive.getValueFor(element), accentFillFocus.getValueFor(element), ContrastTarget.large) });
/** @public @deprecated Not used */
var foregroundOnAccentRestLarge = create("foreground-on-accent-rest-large").withDefault((element) => foregroundOnAccentLargeRecipe.getValueFor(element).evaluate(element).rest);
/** @public @deprecated Not used */
var foregroundOnAccentHoverLarge = create("foreground-on-accent-hover-large").withDefault((element) => foregroundOnAccentLargeRecipe.getValueFor(element).evaluate(element, accentFillHover.getValueFor(element)).hover);
/** @public @deprecated Not used */
var foregroundOnAccentActiveLarge = create("foreground-on-accent-active-large").withDefault((element) => foregroundOnAccentLargeRecipe.getValueFor(element).evaluate(element, accentFillActive.getValueFor(element)).active);
/** @public @deprecated Not used */
var foregroundOnAccentFocusLarge = create("foreground-on-accent-focus-large").withDefault((element) => foregroundOnAccentLargeRecipe.getValueFor(element).evaluate(element, accentFillFocus.getValueFor(element)).focus);
/** @public @deprecated Not used */
var neutralFillInverseRestDelta = create("neutral-fill-inverse-rest-delta").withDefault(0);
/** @public @deprecated Not used */
var neutralFillInverseHoverDelta = create("neutral-fill-inverse-hover-delta").withDefault(-3);
/** @public @deprecated Not used */
var neutralFillInverseActiveDelta = create("neutral-fill-inverse-active-delta").withDefault(7);
/** @public @deprecated Not used */
var neutralFillInverseFocusDelta = create("neutral-fill-inverse-focus-delta").withDefault(0);
/** @deprecated Not used */
...
/** @public @deprecated Not used */
var neutralFillInverseRecipe = createNonCss("neutral-fill-inverse-recipe").withDefault({ evaluate: (element, reference) => neutralFillInverse(neutralPalette.getValueFor(element), reference || fillColor.getValueFor(element), neutralFillInverseRestDelta.getValueFor(element), neutralFillInverseHoverDelta.getValueFor(element), neutralFillInverseActiveDelta.getValueFor(element), neutralFillInverseFocusDelta.getValueFor(element)) });
/** @public @deprecated Not used */
var neutralFillInverseRest = create("neutral-fill-inverse-rest").withDefault((element) => neutralFillInverseRecipe.getValueFor(element).evaluate(element).rest);
/** @public @deprecated Not used */
var neutralFillInverseHover = create("neutral-fill-inverse-hover").withDefault((element) => neutralFillInverseRecipe.getValueFor(element).evaluate(element).hover);
/** @public @deprecated Not used */
var neutralFillInverseActive = create("neutral-fill-inverse-active").withDefault((element) => neutralFillInverseRecipe.getValueFor(element).evaluate(element).active);
/** @public @deprecated Not used */
var neutralFillInverseFocus = create("neutral-fill-inverse-focus").withDefault((element) => neutralFillInverseRecipe.getValueFor(element).evaluate(element).focus);
...
var neutralLayerL1 = neutralLayer1;
/** @public @deprecated Use neutralLayer2 */
var neutralLayerL2 = neutralLayer2;
/** @public @deprecated Use neutralLayer3 */
var neutralLayerL3 = neutralLayer3;
/** @public @deprecated Use neutralLayer4 */
var neutralLayerL4 = neutralLayer4;
...
var neutralFocus = focusStrokeOuter;
/** @public @deprecated Use focusStrokeInner */
var neutralFocusInnerAccent = focusStrokeInner;
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/styles/patterns/type-ramp.js
/** @public */
...
`;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/accordion/accordion.styles.js
var accordionStyles$1 = (context, definition) => css`
    ${display("flex")} :host {
  ...
  `;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/styles/focus.js
/**
...
`;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/styles/size.js
/**
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/accordion/accordion-item/accordion-item.styles.js
var neutralFillStealthRestOnNeutralFillLayerRest = DesignToken.create("neutral-fill-stealth-rest-on-neutral-fill-layer-rest").withDefault((target) => {
	const baseRecipe = neutralFillLayerRecipe.getValueFor(target);
	return neutralFillStealthRecipe.getValueFor(target).evaluate(target, baseRecipe.evaluate(target).rest).rest;
});
var neutralFillStealthHoverOnNeutralFillLayerRest = DesignToken.create("neutral-fill-stealth-hover-on-neutral-fill-layer-rest").withDefault((target) => {
	const baseRecipe = neutralFillLayerRecipe.getValueFor(target);
	return neutralFillStealthRecipe.getValueFor(target).evaluate(target, baseRecipe.evaluate(target).rest).hover;
});
var neutralFillStealthActiveOnNeutralFillLayerRest = DesignToken.create("neutral-fill-stealth-active-on-neutral-fill-layer-rest").withDefault((target) => {
	const baseRecipe = neutralFillLayerRecipe.getValueFor(target);
	return neutralFillStealthRecipe.getValueFor(target).evaluate(target, baseRecipe.evaluate(target).rest).active;
});
var accordionItemStyles$1 = (context, definition) => css`
    ${display("flex")} :host {
      box-sizing: border-box;
      ${typeRampBase};
      flex-direction: column;
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/accordion/accordion-item/index.js
/**
...
* @public
*/
var accordionItemStyles = accordionItemStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/accordion/index.js
/**
...
* @public
*/
var accordionStyles = accordionStyles$1;
//#endregion
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/styles/direction.js
/**
...
var DirectionalStyleSheetBehavior = class {
	...
	*/
	unbind(source) {
		...
		if (cache) direction.unsubscribe(cache);
	}
	attach(source) {
...
var DirectionalStyleSheetBehaviorSubscription = class {
	...
	attach(direction) {
		if (this.attached !== this[direction]) {
			if (this.attached !== null) this.source.$fastController.removeStyles(this.attached);
			this.attached = this[direction];
			if (this.attached !== null) this.source.$fastController.addStyles(this.attached);
		}
	}
};
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/styles/elevation.js
/**
...
var elevationShadowRecipe = DesignToken.create({
	...
	cssCustomPropertyName: null
}).withDefault({ evaluate: (element, size, reference) => {
	let ambientOpacity = .12;
	let directionalOpacity = .14;
	if (size > 16) {
		ambientOpacity = .2;
		directionalOpacity = .24;
	}
	return `${`0 0 2px rgba(0, 0, 0, ${ambientOpacity})`}, ${`0 calc(${size} * 0.5px) calc((${size} * 1px)) rgba(0, 0, 0, ${directionalOpacity})`}`;
} });
/** @public */
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/styles/patterns/button.styles.js
/**
* The base styles for button controls, without `appearance` visual differences.
*
* @internal
*/
var baseButtonStyles = (context, definition, interactivitySelector, nonInteractivitySelector = "[disabled]") => css`
    ${display("inline-flex")}
...
* @internal
*/
var NeutralButtonStyles = (context, definition, interactivitySelector, nonInteractivitySelector = "[disabled]") => css`
    .control {
...
* @internal
*/
var AccentButtonStyles = (context, definition, interactivitySelector, nonInteractivitySelector = "[disabled]") => css`
    .control {
...
* @internal
*/
var HypertextStyles = (context, definition, interactivitySelector, nonInteractivitySelector = "[disabled]") => css`
    :host {
      height: auto;
      font-family: inherit;
      font-size: inherit;
...
* @internal
*/
var LightweightButtonStyles = (context, definition, interactivitySelector, nonInteractivitySelector = "[disabled]") => css`
    :host {
      color: ${accentForegroundRest};
    }
...
* @internal
*/
var OutlineButtonStyles = (context, definition, interactivitySelector, nonInteractivitySelector = "[disabled]") => css`
    .control {
...
* @internal
*/
var StealthButtonStyles = (context, definition, interactivitySelector, nonInteractivitySelector = "[disabled]") => css`
    .control {
      background: ${neutralFillStealthRest};
    }
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/styles/patterns/input.styles.js
var placeholderRest = DesignToken.create("input-placeholder-rest").withDefault((target) => {
	const baseRecipe = neutralFillInputRecipe.getValueFor(target);
	return neutralForegroundHintRecipe.getValueFor(target).evaluate(target, baseRecipe.evaluate(target).rest);
});
var placeholderHover = DesignToken.create("input-placeholder-hover").withDefault((target) => {
	const baseRecipe = neutralFillInputRecipe.getValueFor(target);
	return neutralForegroundHintRecipe.getValueFor(target).evaluate(target, baseRecipe.evaluate(target).hover);
});
var filledPlaceholderRest = DesignToken.create("input-filled-placeholder-rest").withDefault((target) => {
	const baseRecipe = neutralFillSecondaryRecipe.getValueFor(target);
	return neutralForegroundHintRecipe.getValueFor(target).evaluate(target, baseRecipe.evaluate(target).rest);
});
var filledPlaceholderHover = DesignToken.create("input-filled-placeholder-hover").withDefault((target) => {
	const baseRecipe = neutralFillSecondaryRecipe.getValueFor(target);
	return neutralForegroundHintRecipe.getValueFor(target).evaluate(target, baseRecipe.evaluate(target).hover);
});
/**
...
* @internal
*/
var baseInputStyles = (context, definition, logicalControlSelector) => css`
  :host {
    ${typeRampBase}
    color: ${neutralForegroundRest};
    fill: currentcolor;
...
* @internal
*/
var inputStateStyles = (context, definition, logicalControlSelector) => css`
...
* @internal
*/
var inputOutlineStyles = (context, definition, logicalControlSelector, interactivitySelector = ":not([disabled]):not(:focus-within)") => css`
  ${logicalControlSelector} {
...
* @internal
*/
var inputFilledStyles = (context, definition, logicalControlSelector, interactivitySelector = ":not([disabled]):not(:focus-within)") => css`
  ${logicalControlSelector} {
...
* @internal
*/
var inputForcedColorStyles = (context, definition, logicalControlSelector, interactivitySelector = ":not([disabled]):not(:focus-within)") => css`
  :host {
    color: ${SystemColors.ButtonText};
  }
...
`;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/utilities/behaviors.js
/**
...
var Anchor = class extends Anchor$1 {
	...
	*/
	connectedCallback() {
		super.connectedCallback();
		if (!this.appearance) this.appearance = "neutral";
	}
	/**
	...
	*/
	defaultSlottedContentChanged() {
		...
		if (slottedElements.length === 1 && slottedElements[0] instanceof SVGElement) (_a = this.control) === null || _a === void 0 || _a.classList.add("icon-only");
		else (_b = this.control) === null || _b === void 0 || _b.classList.remove("icon-only");
	}
};
...
* @public
*/
var anchorStyles = anchorStyles$1;
/**
...
*/
var fluentAnchor = Anchor.compose({
	baseName: "anchor",
	baseClass: Anchor$1,
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/anchored-region/anchored-region.styles.js
var anchoredRegionStyles$1 = (context, definition) => css`
  :host {
    contain: layout;
    display: block;
  }
`;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/anchored-region/index.js
/**
...
* @public
*/
var anchoredRegionStyles = anchoredRegionStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/badge/badge.styles.js
var badgeStyles$1 = (context, definition) => css`
  ...
  `;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/badge/index.js
/**
* The Fluent Badge class
* @internal
*/
var Badge = class extends Badge$1 {
	constructor() {
		super(...arguments);
		this.appearance = "lightweight";
	}
	appearanceChanged(oldValue, newValue) {
		if (oldValue !== newValue) DOM.queueUpdate(() => {
			this.classList.add(newValue);
			this.classList.remove(oldValue);
		});
	}
};
__decorate([attr({ mode: "fromView" })], Badge.prototype, "appearance", void 0);
/**
...
* @public
*/
var badgeStyles = badgeStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/breadcrumb/breadcrumb.styles.js
var breadcrumbStyles$1 = (context, definition) => css`
  ${display("inline-block")} :host {
    box-sizing: border-box;
    ${typeRampBase};
  }
...
`;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/breadcrumb/index.js
/**
...
* @public
*/
var breadcrumbStyles = breadcrumbStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/breadcrumb-item/breadcrumb-item.styles.js
var breadcrumbItemStyles$1 = (context, definition) => css`
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/breadcrumb-item/index.js
/**
...
* @public
*/
var breadcrumbItemStyles = breadcrumbItemStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/button/button.styles.js
var interactivitySelector$2 = ":not([disabled])";
var nonInteractivitySelector$1 = "[disabled]";
var buttonStyles$1 = (context, definition) => css`
    :host(${interactivitySelector$2}) .control {
      cursor: pointer;
    }

    :host(${nonInteractivitySelector$1}) .control {
      cursor: ${disabledCursor};
    }
    ...
    ${baseButtonStyles(context, definition, interactivitySelector$2, nonInteractivitySelector$1)}
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/button/index.js
/**
* The Fluent button class
* @internal
*/
var Button = class extends Button$1 {
	appearanceChanged(oldValue, newValue) {
		if (oldValue !== newValue) {
			this.classList.add(newValue);
			this.classList.remove(oldValue);
	...
	*/
	defaultSlottedContentChanged() {
		const slottedElements = this.defaultSlottedContent.filter((x) => x.nodeType === Node.ELEMENT_NODE);
		if (slottedElements.length === 1 && slottedElements[0] instanceof SVGElement) this.control.classList.add("icon-only");
		else this.control.classList.remove("icon-only");
	}
};
...
* {@link https://developer.mozilla.org/en-US/docs/Web/API/ShadowRoot/delegatesFocus | delegatesFocus}
*/
var fluentButton = Button.compose({
	baseName: "button",
	baseClass: Button$1,
...
* @public
*/
var buttonStyles = buttonStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/calendar/calendar.styles.js
/**
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/calendar/index.js
/**
* Updated Calendar class that is readonly by default
*/
var Calendar = class extends Calendar$1 {
	constructor() {
		super(...arguments);
		this.readonly = true;
	}
};
__decorate([attr({ converter: booleanConverter })], Calendar.prototype, "readonly", void 0);
/**
...
*/
var fluentCalendar = Calendar.compose({
	baseName: "calendar",
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/card/card.styles.js
var cardStyles$1 = (context, definition) => css`
    ${display("block")} :host {
      display: block;
      contain: content;
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/card/index.js
/**
* @public
*/
var Card = class extends Card$1 {
	...
	*/
	handleChange(source, propertyName) {
		if (!this.cardFillColor) fillColor.setValueFor(this, (target) => neutralFillLayerRecipe.getValueFor(target).evaluate(target, fillColor.getValueFor(source)).rest);
	}
	connectedCallback() {
		super.connectedCallback();
...
* @public
*/
var cardStyles = cardStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/checkbox/checkbox.styles.js
var checkboxStyles$1 = (context, definition) => css`
    ${display("inline-flex")} :host {
      align-items: center;
      outline: none;
      ${""} user-select: none;
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/checkbox/index.js
/**
...
* @public
*/
var checkboxStyles = checkboxStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/select/select.styles.js
var logicalControlSelector$5 = ".control";
var interactivitySelector$1 = ":not([disabled]):not([open])";
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/combobox/index.js
/**
* The Fluent combobox class
* @internal
*/
var Combobox = class extends Combobox$1 {
	/**
	* @internal
	*/
	appearanceChanged(oldValue, newValue) {
	...
	*/
	connectedCallback() {
		super.connectedCallback();
		if (!this.appearance) this.appearance = "outline";
		if (this.listbox) fillColor.setValueFor(this.listbox, neutralLayerFloating);
	}
};
__decorate([attr({ mode: "fromView" })], Combobox.prototype, "appearance", void 0);
/**
...
*
*/
var fluentCombobox = Combobox.compose({
	baseName: "combobox",
...
* @public
*/
var comboboxStyles = comboboxStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/data-grid/data-grid.styles.js
var dataGridStyles$1 = (context, definition) => css`
  :host {
    display: flex;
    position: relative;
    flex-direction: column;
...
`;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/data-grid/data-grid-row.styles.js
var dataGridRowStyles$1 = (context, definition) => css`
    :host {
      display: grid;
      padding: 1px 0;
      box-sizing: border-box;
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/data-grid/data-grid-cell.styles.js
var dataGridCellStyles$1 = (context, definition) => css`
    :host {
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/data-grid/index.js
/**
...
* @public
*/
var dataGridCellStyles = dataGridCellStyles$1;
/**
...
* @public
*/
var dataGridRowStyles = dataGridRowStyles$1;
/**
...
* @public
*/
var dataGridStyles = dataGridStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/design-system-provider/index.js
/**
...
function designToken(token) {
	return (source, key) => {
		source[key + "Changed"] = function(prev, next) {
			if (next !== void 0 && next !== null) token.setValueFor(this, next);
			else token.deleteValueFor(this);
		};
	};
}
/**
...
var DesignSystemProvider = class extends FoundationElement {
	constructor() {
		...
		*/
		this.noPaint = false;
		const subscriber = { handleChange: this.noPaintChanged.bind(this) };
	...
	noPaintChanged() {
		if (!this.noPaint && (this.fillColor !== void 0 || this.baseLayerLuminance)) this.$fastController.addStyles(backgroundStyles);
		else this.$fastController.removeStyles(backgroundStyles);
	}
};
__decorate([attr({
	attribute: "no-paint",
...
__decorate([attr({ converter: nullableNumberConverter }), designToken(density)], DesignSystemProvider.prototype, "density", void 0);
__decorate([attr({
	attribute: "design-unit",
	converter: nullableNumberConverter
}), designToken(designUnit)], DesignSystemProvider.prototype, "designUnit", void 0);
__decorate([attr({ attribute: "direction" }), designToken(direction)], DesignSystemProvider.prototype, "direction", void 0);
__decorate([attr({
...
__decorate([attr({ attribute: "type-ramp-minus-2-font-size" }), designToken(typeRampMinus2FontSize)], DesignSystemProvider.prototype, "typeRampMinus2FontSize", void 0);
__decorate([attr({ attribute: "type-ramp-minus-2-line-height" }), designToken(typeRampMinus2LineHeight)], DesignSystemProvider.prototype, "typeRampMinus2LineHeight", void 0);
__decorate([attr({ attribute: "type-ramp-minus-1-font-size" }), designToken(typeRampMinus1FontSize)], DesignSystemProvider.prototype, "typeRampMinus1FontSize", void 0);
__decorate([attr({ attribute: "type-ramp-minus-1-line-height" }), designToken(typeRampMinus1LineHeight)], DesignSystemProvider.prototype, "typeRampMinus1LineHeight", void 0);
__decorate([attr({ attribute: "type-ramp-base-font-size" }), designToken(typeRampBaseFontSize)], DesignSystemProvider.prototype, "typeRampBaseFontSize", void 0);
__decorate([attr({ attribute: "type-ramp-base-line-height" }), designToken(typeRampBaseLineHeight)], DesignSystemProvider.prototype, "typeRampBaseLineHeight", void 0);
__decorate([attr({ attribute: "type-ramp-plus-1-font-size" }), designToken(typeRampPlus1FontSize)], DesignSystemProvider.prototype, "typeRampPlus1FontSize", void 0);
__decorate([attr({ attribute: "type-ramp-plus-1-line-height" }), designToken(typeRampPlus1LineHeight)], DesignSystemProvider.prototype, "typeRampPlus1LineHeight", void 0);
__decorate([attr({ attribute: "type-ramp-plus-2-font-size" }), designToken(typeRampPlus2FontSize)], DesignSystemProvider.prototype, "typeRampPlus2FontSize", void 0);
__decorate([attr({ attribute: "type-ramp-plus-2-line-height" }), designToken(typeRampPlus2LineHeight)], DesignSystemProvider.prototype, "typeRampPlus2LineHeight", void 0);
__decorate([attr({ attribute: "type-ramp-plus-3-font-size" }), designToken(typeRampPlus3FontSize)], DesignSystemProvider.prototype, "typeRampPlus3FontSize", void 0);
__decorate([attr({ attribute: "type-ramp-plus-3-line-height" }), designToken(typeRampPlus3LineHeight)], DesignSystemProvider.prototype, "typeRampPlus3LineHeight", void 0);
__decorate([attr({ attribute: "type-ramp-plus-4-font-size" }), designToken(typeRampPlus4FontSize)], DesignSystemProvider.prototype, "typeRampPlus4FontSize", void 0);
__decorate([attr({ attribute: "type-ramp-plus-4-line-height" }), designToken(typeRampPlus4LineHeight)], DesignSystemProvider.prototype, "typeRampPlus4LineHeight", void 0);
__decorate([attr({ attribute: "type-ramp-plus-5-font-size" }), designToken(typeRampPlus5FontSize)], DesignSystemProvider.prototype, "typeRampPlus5FontSize", void 0);
__decorate([attr({ attribute: "type-ramp-plus-5-line-height" }), designToken(typeRampPlus5LineHeight)], DesignSystemProvider.prototype, "typeRampPlus5LineHeight", void 0);
__decorate([attr({ attribute: "type-ramp-plus-6-font-size" }), designToken(typeRampPlus6FontSize)], DesignSystemProvider.prototype, "typeRampPlus6FontSize", void 0);
__decorate([attr({ attribute: "type-ramp-plus-6-line-height" }), designToken(typeRampPlus6LineHeight)], DesignSystemProvider.prototype, "typeRampPlus6LineHeight", void 0);
__decorate([attr({
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/dialog/dialog.styles.js
var dialogStyles$1 = (context, definition) => css`
  :host([hidden]) {
    display: none;
  }
...
`;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/dialog/index.js
/**
...
* @public
*/
var dialogStyles = dialogStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/divider/divider.styles.js
var dividerStyles$1 = (context, definition) => css`
    ${display("block")} :host {
  ...
  `;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/divider/index.js
/**
...
* @public
*/
var dividerStyles = dividerStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/flipper/flipper.styles.js
var flipperStyles$1 = (context, definition) => css`
    ${display("inline-flex")} :host {
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/flipper/index.js
/**
...
* @public
*/
var flipperStyles = flipperStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/horizontal-scroll/horizontal-scroll.styles.js
...
* @public
*/
var horizontalScrollStyles$1 = (context, definition) => css`
  ${display("block")} :host {
    --scroll-align: center;
...
`;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/horizontal-scroll/index.js
/**
* @internal
*/
var HorizontalScroll = class extends HorizontalScroll$1 {
	/**
	* @public
	*/
	connectedCallback() {
		super.connectedCallback();
		if (this.view !== "mobile") this.$fastController.addStyles(ActionsStyles);
	}
};
/**
...
*/
var fluentHorizontalScroll = HorizontalScroll.compose({
...
* @public
*/
var horizontalScrollStyles = horizontalScrollStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/listbox/listbox.styles.js
var listboxStyles$1 = (context, definition) => css`
    ${display("inline-flex")} :host {
  ...
  `;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/listbox/index.js
var Listbox = class extends Listbox$1 {};
/**
...
*
*/
var fluentListbox = Listbox.compose({
	baseName: "listbox",
...
* @public
*/
var listboxStyles = listboxStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/listbox-option/listbox-option.styles.js
var optionStyles = (context, definition) => css`
  ...
  `.withBehaviors(new DirectionalStyleSheetBehavior(null, css`
    ...
    `), forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/listbox-option/index.js
/**
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/menu/menu.styles.js
var menuStyles$1 = (context, definition) => css`
    ${display("block")} :host {
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/menu/index.js
/**
* The Fluent menu class
* @public
*/
var Menu = class extends Menu$1 {
	/**
	* @internal
	*/
	connectedCallback() {
...
* @public
*/
var menuStyles = menuStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/menu-item/menu-item.styles.js
var menuItemStyles$1 = (context, definition) => css`
    ${display("grid")} :host {
      contain: layout;
      overflow: visible;
      ${typeRampBase}
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/menu-item/index.js
/**
...
* @public
*/
var menuItemStyles = menuItemStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/number-field/number-field.styles.js
var logicalControlSelector$3 = ".root";
var numberFieldStyles$1 = (context, definition) => css`
    ${display("inline-block")}

    ${baseInputStyles(context, definition, logicalControlSelector$3)}

    ${inputStateStyles(context, definition, logicalControlSelector$3)}
  ...
  `.withBehaviors(appearanceBehavior("outline", inputOutlineStyles(context, definition, logicalControlSelector$3)), appearanceBehavior("filled", inputFilledStyles(context, definition, logicalControlSelector$3)), forcedColorsStylesheetBehavior(inputForcedColorStyles(context, definition, logicalControlSelector$3)));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/number-field/index.js
/**
* The Fluent number field class
* @internal
*/
var NumberField = class extends NumberField$1 {
	/**
	* @internal
	*/
	connectedCallback() {
...
* @public
*/
var numberFieldStyles = numberFieldStyles$1;
/**
...
* {@link https://developer.mozilla.org/en-US/docs/Web/API/ShadowRoot/delegatesFocus | delegatesFocus}
*/
var fluentNumberField = NumberField.compose({
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/progress/progress/progress.styles.js
var progressStyles$1 = (context, definition) => css`
    ${display("flex")} :host {
      align-items: center;
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/progress/progress/index.js
/**
* Progress base class
* @public
*/
...
* @public
*/
var progressStyles = progressStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/progress/progress-ring/progress-ring.styles.js
var progressRingStyles$1 = (context, definition) => css`
    ${display("flex")} :host {
      align-items: center;
      height: calc(${heightNumber} * 1px);
      width: calc(${heightNumber} * 1px);
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/progress/progress-ring/index.js
/**
...
* @public
*/
var progressRingStyles = progressRingStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/radio/radio.styles.js
var radioStyles = (context, definition) => css`
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/radio/index.js
/**
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/radio-group/radio-group.styles.js
var radioGroupStyles$1 = (context, definition) => css`
  ${display("flex")} :host {
...
`;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/radio-group/index.js
/**
...
* @public
*/
var radioGroupStyles = radioGroupStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/search/search.template.js
/**
* @public
*/
...
`;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/search/search.styles.js
var logicalControlSelector$2 = ".root";
var clearButtonHover = DesignToken.create("clear-button-hover").withDefault((target) => {
...
var clearButtonActive = DesignToken.create("clear-button-active").withDefault((target) => {
	const buttonRecipe = neutralFillStealthRecipe.getValueFor(target);
	const inputRecipe = neutralFillInputRecipe.getValueFor(target);
	return buttonRecipe.evaluate(target, inputRecipe.evaluate(target).focus).active;
});
var searchStyles$1 = (context, definition) => css`
    ${display("inline-block")}

    ${baseInputStyles(context, definition, logicalControlSelector$2)}

    ${inputStateStyles(context, definition, logicalControlSelector$2)}
    ...
    ::slotted(${context.tagFor(Button$1)}) {
      margin-inline-end: 1px;
    }
  `.withBehaviors(appearanceBehavior("outline", inputOutlineStyles(context, definition, logicalControlSelector$2)), appearanceBehavior("filled", inputFilledStyles(context, definition, logicalControlSelector$2)), forcedColorsStylesheetBehavior(inputForcedColorStyles(context, definition, logicalControlSelector$2)));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/search/index.js
/**
* The Fluent search class
* @internal
*/
var Search = class extends Search$1 {
	constructor() {
		super(...arguments);
		/**
...
* {@link https://developer.mozilla.org/en-US/docs/Web/API/ShadowRoot/delegatesFocus | delegatesFocus}
*/
var fluentSearch = Search.compose({
	baseName: "search",
	baseClass: Search$1,
...
* @public
*/
var searchStyles = searchStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/select/index.js
/**
* The Fluent select class
* @internal
*/
var Select = class extends Select$1 {
	/**
	* @internal
	*/
	appearanceChanged(oldValue, newValue) {
...
__decorate([attr({ mode: "fromView" })], Select.prototype, "appearance", void 0);
/**
...
*
*/
var fluentSelect = Select.compose({
	baseName: "select",
	baseClass: Select$1,
...
* @public
*/
var selectStyles = selectStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/skeleton/skeleton.styles.js
var skeletonStyles$1 = (context, definition) => css`
    ${display("block")} :host {
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/skeleton/index.js
/**
...
* @public
*/
var skeletonStyles = skeletonStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/slider/slider.styles.js
var sliderStyles$1 = (context, definition) => css`
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/slider/index.js
/**
...
* @public
*/
var sliderStyles = sliderStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/slider-label/slider-label.styles.js
var sliderLabelStyles$1 = (context, definition) => css`
    ${display("block")} :host {
      ${typeRampMinus1}
    }
    .root {
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/slider-label/index.js
/**
...
* @public
*/
var sliderLabelStyles = sliderLabelStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/switch/switch.styles.js
var switchStyles$1 = (context, definition) => css`
    :host([hidden]) {
      display: none;
    }
  ...
  `.withBehaviors(new DirectionalStyleSheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/switch/index.js
/**
...
* @public
*/
var switchStyles = switchStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/tabs/tabs.styles.js
var tabsStyles$1 = (context, definition) => css`
      ${display("grid")} :host {
        box-sizing: border-box;
        ${typeRampBase}
        color: ${neutralForegroundRest};
    ...
    `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/tabs/tab/tab.styles.js
var tabStyles$1 = (context, definition) => css`
      ${display("inline-flex")} :host {
        box-sizing: border-box;
        ${typeRampBase}
    ...
    `.withBehaviors(forcedColorsStylesheetBehavior(css`
        ...
        `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/tabs/tab/index.js
/**
...
* @public
*/
var tabStyles = tabStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/tabs/tab-panel/tab-panel.styles.js
var tabPanelStyles$1 = (context, definition) => css`
  ${display("block")} :host {
    box-sizing: border-box;
    ${typeRampBase}
    padding: 0 calc((6 + (${designUnit} * 2 * ${density})) * 1px);
...
`;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/tabs/tab-panel/index.js
/**
...
* @public
*/
var tabPanelStyles = tabPanelStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/tabs/index.js
/**
...
* @public
*/
var tabsStyles = tabsStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/text-area/text-area.styles.js
var logicalControlSelector$1 = ".control";
var textAreaStyles$1 = (context, definition) => css`
    ${display("inline-flex")}

    ${baseInputStyles(context, definition, logicalControlSelector$1)}

    ${inputStateStyles(context, definition, logicalControlSelector$1)}
  ...
  `.withBehaviors(appearanceBehavior("outline", inputOutlineStyles(context, definition, logicalControlSelector$1)), appearanceBehavior("filled", inputFilledStyles(context, definition, logicalControlSelector$1)), forcedColorsStylesheetBehavior(inputForcedColorStyles(context, definition, logicalControlSelector$1)));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/text-area/index.js
/**
* The Fluent TextArea class
* @internal
*/
var TextArea = class extends TextArea$1 {
	/**
	* @internal
	*/
	appearanceChanged(oldValue, newValue) {
...
* {@link https://developer.mozilla.org/en-US/docs/Web/API/ShadowRoot/delegatesFocus | delegatesFocus}
*/
var fluentTextArea = TextArea.compose({
	baseName: "text-area",
...
* @public
*/
var textAreaStyles = textAreaStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/text-field/text-field.styles.js
var logicalControlSelector = ".root";
var textFieldStyles$1 = (context, definition) => css`
    ${display("inline-block")}
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/text-field/index.js
/**
* The Fluent text field class
* @internal
*/
var TextField = class extends TextField$1 {
	/**
	* @internal
	*/
	appearanceChanged(oldValue, newValue) {
...
* {@link https://developer.mozilla.org/en-US/docs/Web/API/ShadowRoot/delegatesFocus | delegatesFocus}
*/
var fluentTextField = TextField.compose({
	baseName: "text-field",
...
* @public
*/
var textFieldStyles = textFieldStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/toolbar/toolbar.styles.js
var toolbarStyles = (context, definition) => css`
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/toolbar/index.js
/**
* The Fluent toolbar class
* @internal
*/
var Toolbar = class extends Toolbar$1 {};
/**
...
*/
var fluentToolbar = Toolbar.compose({
	baseName: "toolbar",
	baseClass: Toolbar$1,
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/tooltip/tooltip.styles.js
var tooltipStyles = (context, definition) => css`
  ...
  `.withBehaviors(forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/tooltip/index.js
/**
* The Fluent tooltip class
* @internal
*/
var Tooltip = class extends Tooltip$1 {
	/**
	* @internal
	*/
	connectedCallback() {
...
*/
var fluentTooltip = Tooltip.compose({
	baseName: "tooltip",
	baseClass: Tooltip$1,
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/tree-view/tree-view.styles.js
var treeViewStyles$1 = (context, definition) => css`
  :host([hidden]) {
    display: none;
  }
...
`;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/tree-view/index.js
/**
...
* @public
*/
var treeViewStyles = treeViewStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/tree-item/tree-item.styles.js
var ltr = css`
...
var expandCollapseHover = DesignToken.create("tree-item-expand-collapse-hover").withDefault((target) => {
	const recipe = neutralFillStealthRecipe.getValueFor(target);
	return recipe.evaluate(target, recipe.evaluate(target).hover).hover;
});
var selectedExpandCollapseHover = DesignToken.create("tree-item-expand-collapse-selected-hover").withDefault((target) => {
	const baseRecipe = neutralFillSecondaryRecipe.getValueFor(target);
	return neutralFillStealthRecipe.getValueFor(target).evaluate(target, baseRecipe.evaluate(target).rest).hover;
});
var treeItemStyles$1 = (context, definition) => css`
    ${display("block")} :host {
      contain: content;
      position: relative;
      outline: none;
  ...
  `.withBehaviors(new DirectionalStyleSheetBehavior(ltr, rtl), forcedColorsStylesheetBehavior(css`
      ...
      `));
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/tree-item/index.js
/**
...
* @public
*/
var treeItemStyles = treeItemStyles$1;
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/custom-elements.js
/**
...
//#endregion
//#region node_modules/@fluentui/web-components/dist/esm/fluent-design-system.js
/**
